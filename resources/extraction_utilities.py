...
def get_tools_for_service(mcp_server: FastMCP, service_name: str) -> Dict[str, Callable]:
    ...
    for tool_name, tool_instance in registered_tools.items():
        ...
        if (service_name in tool_name.lower() or 
            (hasattr(tool_instance, 'tags') and service_name in tool_instance.tags) or
            tool_name.startswith(f"list_{service_name}") or
            tool_name.startswith(f"{service_name}_")):
            
            service_tools[tool_name] = tool_instance.fn if hasattr(tool_instance, 'fn') else tool_instance
...
def categorize_tools_by_service(mcp_server: FastMCP) -> Dict[str, List[str]]:
    ...
    for tool_name, tool_instance in registered_tools.items():
        ...
        for service in service_categories.keys():
            if (service in tool_name.lower() or 
                tool_name.startswith(f"list_{service}") or
                tool_name.startswith(f"{service}_") or
                (hasattr(tool_instance, 'tags') and service in getattr(tool_instance, 'tags', []))):
...
def get_list_tools_mapping() -> Dict[str, Dict[str, str]]:
    ...
    return {
        "gmail": {
            "filters": "list_gmail_filters",
            "labels": "list_gmail_labels"
        },
        "drive": {
        ...
        "forms": {
            "form_responses": "list_form_responses"
        },
        "sheets": {
            "spreadsheets": "list_spreadsheets"
        },
        "docs": {
            "documents": "list_docs_in_folder"
        },
        "chat": {
            "spaces": "list_spaces"
        },
        "photos": {
            "albums": "list_photos_albums"
        }
    }
