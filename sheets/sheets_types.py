...
class SpreadsheetInfo(BaseModel):
    ...
    modifiedTime: Optional[str] = Field(None, description="Last modification time in ISO format")
    webViewLink: Optional[str] = Field(None, description="Web view URL for the spreadsheet")
...
class SpreadsheetListResponse(BaseModel):
    ...
    items: List[SpreadsheetInfo] = Field(..., description="List of spreadsheet information")
...
class SpreadsheetDetailsResponse(BaseModel):
    ...
    spreadsheetUrl: Optional[str] = Field(None, description="Web URL for the spreadsheet")
    error: Optional[str] = Field(None, description="Error message if operation failed")
...
class SheetModifyResponse(BaseModel):
    ...
    operation: str = Field(..., description="Type of operation performed ('update', 'clear')")
...
class CreateSpreadsheetResponse(BaseModel):
    ...
    spreadsheetId: str = Field(..., description="Unique identifier for the created spreadsheet")
...
class FormatCellsResponse(BaseModel):
    ...
    formattingApplied: dict = Field(..., description="Summary of formatting options applied")
...
class ConditionalFormattingResponse(BaseModel):
    ...
    spreadsheetId: str = Field(..., description="Unique identifier for the spreadsheet")
    sheetId: int = Field(..., description="Sheet ID where conditional formatting was added")
...
class FormatRangeResponse(BaseModel):
    ...
    sheetId: int = Field(..., description="Sheet ID where formatting was applied")
    range: str = Field(..., description="Cell range that was formatted")
    requestsApplied: int = Field(..., description="Number of formatting requests successfully applied")
    formattingDetails: dict = Field(..., description="Details of all formatting applied")
    success: bool = Field(..., description="Whether the formatting succeeded")
    message: str = Field(..., description="Success or error message")
    error: Optional[str] = Field(None, description="Error message if operation failed")