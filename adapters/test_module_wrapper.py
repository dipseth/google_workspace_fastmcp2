...
def test_search(wrapper):
    ...
    for i, result in enumerate(results):
        logger.info(f"{i+1}. {result['path']} ({result['type']}) - Score: {result['score']:.4f}")
        if result['docstring']:
...
def test_performance(module_name="json"):
    ...
    logger.info(f"Average time per component: {(wrap_time / component_count) * 1000:.2f} ms")
    ...
    for query in queries:
        ...
        logger.info(f"Search for '{query}' took {search_time:.4f} seconds, found {len(results)} results")
    
    logger.info(f"Average search time: {(total_search_time / len(queries)):.4f} seconds")
