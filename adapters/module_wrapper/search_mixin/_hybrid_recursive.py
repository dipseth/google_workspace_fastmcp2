...
def search_hybrid_recursive(
    ...
    try:
        ...
        for cycle in range(max_cycles):
            ...
            if (
                cycle == 0
                and not component_paths
                and self._learned_feature_version in (2, 3, 4, 5)
            ):
            ...
            # --- Cycle tracking ---
            top1_name = (
                (scored[0][1][4].payload or {}).get("name", "?") if scored else "?"
            )
            ...
            if is_unified or is_dual:
                ...
                form_top1_name = (points_data[form_top1_idx][4].payload or {}).get(
                    "name", "?"
                )
                content_top1_name = (
                    points_data[content_top1_idx][4].payload or {}
                ).get("name", "?")
            ...
            if has_halt_head and halt_probs_list and cycle > 0:
                ...
                form_converged = (
                    margin > halt_margin or form_top1_name == prev_top1_name
                )
                content_converged = (
            ...
            prev_content_top1_name = (
                content_top1_name if (is_unified or is_dual) else None
            )
            ...
            if cycle < max_cycles - 1:
                ...
                bottom_ids = (
                    [pt.id for _, (_, _, _, _, pt) in scored[-3:]]
                    if len(scored) > 5
                    else []
                )
                ...
                try:
                    ...
                    logger.info(
                        ...
                        len(top_ids),
                        len(bottom_ids),
                        bool(content_rec_prefetch),
                        len(rec_results.points),
                    )
                    _next_cycle_points = rec_results.points
                except Exception as e:
                    logger.debug(
                        "RecommendQuery prefetch failed (%s), falling back to vector blending",
                        e,
                    )
                    ...
                    if top_comp_vecs:
                        ...
                        refined = (
                            alpha * np.array(query_colbert_orig[0])
                            + (1 - alpha) * top_mean
                        ).tolist()
