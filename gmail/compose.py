...
from .gmail_types import (
    ...
    GmailMessageContentResponse, GmailMessageData
)
...
) -> GmailMessageContentResponse:
    """
    Retrieve Gmail message content with flexible input options.
    
    This unified tool can handle:
    - Single message retrieval (message_id)
    - Batch message retrieval (message_ids, up to 100)
    - Complete thread retrieval (thread_id)
    
    Args:
        user_google_email: The user's Google email address for Gmail access. 
                          If None, uses the current authenticated user from FastMCP context.
        message_id: Single Gmail message ID to retrieve (mutually exclusive with message_ids/thread_id)
        message_ids: List of Gmail message IDs for batch retrieval (max 100, mutually exclusive with message_id/thread_id)
        thread_id: Gmail thread ID to retrieve all messages in the conversation (mutually exclusive with message_id/message_ids)
        format: Message format - 'full' includes message body, 'metadata' only includes headers
        
    Returns:
        GmailMessageContentResponse: Structured response containing message(s) content
        
    Examples:
        # Single message
        get_gmail_message_content(message_id="msg_123")
        
        # Batch messages
        get_gmail_message_content(message_ids=["msg_123", "msg_456", "msg_789"])
        
        # Complete thread
        get_gmail_message_content(thread_id="thread_abc")
        
        # Metadata only for performance
        get_gmail_message_content(message_ids=["msg_123", "msg_456"], format="metadata")
    """
    # Input validation - ensure only one type of input is provided
    inputs_provided = [bool(message_id), bool(message_ids), bool(thread_id)]
    if sum(inputs_provided) != 1:
        return GmailMessageContentResponse(
            success=False,
            messages=[],
            total_count=0,
            ...
            error="Exactly one of message_id, message_ids, or thread_id must be provided"
        )

    # Validate message_ids limit
    if message_ids and len(message_ids) > 100:
        return GmailMessageContentResponse(
            success=False,
            messages=[],
            total_count=0,
            ...
            format=format,
            user_email=user_google_email or "",
            error="Maximum 100 message IDs allowed per batch request"
        )

    logger.info(f"[get_gmail_message_content] Email: '{user_google_email}', format: {format}")

    try:
        gmail_service = await _get_gmail_service_with_fallback(user_google_email)
        messages_data: List[GmailMessageData] = []
        request_type = ""
        ...
        if message_id:
            ...
            request_type = "single"
            logger.info(f"[get_gmail_message_content] Single message: {message_id}")
            
            message_data = await _fetch_single_message(gmail_service, message_id, format)
            if message_data:
            ...
            request_type = "batch"
            logger.info(f"[get_gmail_message_content] Batch messages: {len(message_ids)} IDs")
            
            messages_data = await _fetch_batch_messages(gmail_service, message_ids, format)

        elif thread_id:
            # Thread retrieval
            request_type = "thread"
            actual_thread_id = thread_id
            logger.info(f"[get_gmail_message_content] Thread: {thread_id}")
            
            messages_data = await _fetch_thread_messages(gmail_service, thread_id, format)
...
async def _fetch_single_message(gmail_service, message_id: str, format: str) -> Optional[GmailMessageData]:
    """Fetch a single Gmail message and convert to unified format."""
    try:
        # Get message with appropriate format
        api_format = "full" if format == "full" else "metadata"
        headers_list = ["Subject", "From", "To", "Cc", "Bcc", "Date"] if format == "metadata" else None
        ...
        return _convert_message_to_unified_format(message, format)

    except Exception as e:
        logger.error(f"Error fetching single message {message_id}: {e}")
        return None


async def _fetch_batch_messages(gmail_service, message_ids: List[str], format: str) -> List[GmailMessageData]:
    """Fetch multiple Gmail messages using batch API and convert to unified format."""
    messages_data: List[GmailMessageData] = []
    ...
    for chunk_start in range(0, len(message_ids), 100):
        ...
        try:
            ...
            logger.warning(f"Batch API failed, falling back to individual requests: {batch_error}")
        ...
        for mid in chunk_ids:
            ...
            if entry["error"] or not entry["data"]:
                logger.warning(f"Failed to fetch message {mid}: {entry.get('error', 'No data')}")
                continue
                
            message_data = _convert_message_to_unified_format(entry["data"], format)
            if message_data:
                messages_data.append(message_data)
...
async def _fetch_thread_messages(gmail_service, thread_id: str, format: str) -> List[GmailMessageData]:
    """Fetch all messages in a Gmail thread and convert to unified format."""
    try:
        # Get the complete thread
        thread_response = await asyncio.to_thread(
            gmail_service.users().threads().get(
                userId="me", 
                id=thread_id, 
                format="full"
            ).execute
        ...
        for message in messages:
            message_data = _convert_message_to_unified_format(message, format)
            if message_data:
                messages_data.append(message_data)
...
def _convert_message_to_unified_format(message: dict, format: str) -> Optional[GmailMessageData]:
    """Convert Gmail API message response to unified GmailMessageData format."""
    try:
        ...
        headers = _extract_headers(payload, ["Subject", "From", "To", "Cc", "Bcc", "Date"])
        
        # Parse recipients
        to_recipients = _parse_recipients(headers.get("To", ""))
        cc_recipients = _parse_recipients(headers.get("Cc", ""))
        bcc_recipients = _parse_recipients(headers.get("Bcc", ""))

        # Create base message data
        message_data: GmailMessageData = {
            "message_id": message_id,
            "thread_id": thread_id,
        ...
        if format == "full":
            ...
            # Try to extract HTML body if available
            try:
                html_body = _extract_html_body(payload)
                if html_body:
                    message_data["body_html"] = html_body
            except:
                pass  # HTML extraction is optional

        # Add labels if available
        label_ids = message.get("labelIds", [])
        if label_ids:
            message_data["labels"] = label_ids

        # Add all headers as metadata
        message_data["headers"] = headers

        return message_data

    except Exception as e:
        logger.error(f"Error converting message to unified format: {e}")
        return None


def _parse_recipients(recipients_str: str) -> List[str]:
    """Parse a recipients string into a list of email addresses."""
    if not recipients_str:
        return []
    
    try:
        # Use the existing utility function if available
        emails = extract_email_addresses(recipients_str)
...
def setup_compose_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool(
        ...
        description="Retrieve Gmail message content with flexible input options - single message, batch messages, or complete thread",
        tags={"gmail", "message", "content", "batch", "thread", "unified"},
        annotations={
            "title": "Gmail Message Content (Unified)",
    ...
    async def get_gmail_message_content_tool(
        user_google_email: UserGoogleEmail = None,
        message_id: Annotated[Optional[str], Field(description="Single Gmail message ID to retrieve (mutually exclusive with message_ids/thread_id)")] = None,
        message_ids: Annotated[Optional[List[str]], Field(description="List of Gmail message IDs for batch retrieval (max 100, mutually exclusive with message_id/thread_id)")] = None,
        thread_id: Annotated[Optional[str], Field(description="Gmail thread ID to retrieve all messages in the conversation (mutually exclusive with message_id/message_ids)")] = None,
        ...
        """
        Unified Gmail message content retrieval with flexible input options.
        
        This unified tool replaces separate single/batch/thread tools and provides:
        - Single message retrieval (message_id)
        - Batch message retrieval (message_ids, up to 100)
        - Complete thread retrieval (thread_id)
        - Flexible format options (full content vs metadata only)
        
        Features:
        - Efficient batch processing using Gmail's batch API
        - Thread support for complete conversation retrieval
        - Unified response format regardless of input type
        - Performance optimization with metadata-only mode
        - Auto-injection of user context via middleware
        - Comprehensive error handling with partial success support
        
        Input Validation:
        - Exactly one of message_id, message_ids, or thread_id must be provided
        - message_ids limited to 100 messages per request (Gmail batch API limit)
        - Invalid inputs return structured error responses
        
        Response Structure:
        - Consistent GmailMessageContentResponse format
        - Contains array of GmailMessageData objects
        - Includes request type, total count, and success indicators
        - Machine-readable format with rich metadata
        
        Args:
            user_google_email: User's email address (auto-injected if None)
            message_id: Single message ID to retrieve
            message_ids: List of message IDs for batch retrieval (max 100)
            thread_id: Thread ID for complete conversation retrieval
            format: Content format ('full' with body, 'metadata' headers only)
            
        Returns:
        GmailMessageContentResponse: Unified structured response with message(s) content
        ...
        get_gmail_message_content(message_ids=["msg_123", "msg_456"], format="metadata")
        """
        return await get_gmail_message_content(user_google_email, message_id, message_ids, thread_id, format)