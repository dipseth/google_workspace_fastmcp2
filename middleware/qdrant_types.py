...
class QdrantCollectionInfo(BaseModel):
    ...
    error: Optional[str] = Field(
        default=None, description="Error message if collection info failed"
    )
...
class QdrantCollectionsListResponse(BaseModel):
    ...
    qdrant_enabled: bool = Field(
        description="Whether Qdrant is available and connected"
    )
    qdrant_url: Optional[str] = Field(
        description="The discovered or configured Qdrant URL"
    )
    total_collections: int = Field(description="Number of collections found")
    collections: List[QdrantCollectionInfo] = Field(
        description="List of collections with their info"
    )
    config: Dict[str, Any] = Field(
        default_factory=dict, description="Current Qdrant configuration"
    )
...
class QdrantCollectionDetailsResponse(BaseModel):
    ...
    collection_info: Optional[Dict[str, Any]] = Field(
        description="Detailed collection information if it exists"
    )
...
class QdrantStoredResponse(BaseModel):
    ...
    response_data: Union[Dict[str, Any], str] = Field(
...
class QdrantRecentResponsesResponse(BaseModel):
    ...
    responses_shown: int = Field(
        description="Number of responses included in this response"
    )
    responses: List[QdrantStoredResponse] = Field(
...
class QdrantSearchResult(BaseModel):
    ...
    payload: Dict[str, Any] = Field(
        default_factory=dict, description="Raw payload data"
    )
...
class QdrantSearchResponse(BaseModel):
    ...
    collection: Optional[str] = Field(
        default=None, description="Collection name if collection-specific search"
    )
...
class QdrantErrorResponse(BaseModel):
    ...
    qdrant_enabled: bool = Field(
        default=False, description="Whether Qdrant is available"
    )
...
class QdrantNearbyPoint(BaseModel):
    ...
    time_offset_seconds: Optional[float] = Field(
        default=None, description="Time difference in seconds from the main point"
    )
    user_email: Optional[str] = Field(default=None, description="User email")
    session_id: Optional[str] = Field(default=None, description="Session ID")
    same_session: bool = Field(
        default=False, description="Whether this point is from the same session"
    )
...
class QdrantPointDetailsResponse(BaseModel):
    ...
    vector_available: bool = Field(
        default=False, description="Whether vector data is included"
    )
    vector_size: Optional[int] = Field(
        default=None, description="Size of the vector if available"
    )
    # Extracted common fields for easy access
    tool_name: Optional[str] = Field(default=None, description="Tool name from payload")
    user_email: Optional[str] = Field(
        default=None, description="User email from payload"
    )
    timestamp: Optional[str] = Field(default=None, description="Timestamp from payload")
    session_id: Optional[str] = Field(
        default=None, description="Session ID from payload"
    )
    payload_type: Optional[str] = Field(
        default=None, description="Payload type from payload"
    )
    compressed: bool = Field(default=False, description="Whether data is compressed")
    # Decompressed data if available
    response_data: Optional[Union[Dict[str, Any], str]] = Field(
        default=None, description="Decompressed response data"
    )
    # Related points for context
    nearby_points: Optional[List[QdrantNearbyPoint]] = Field(
        default=None, description="Two nearest points by timestamp for context"
    )
