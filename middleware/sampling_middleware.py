...
from middleware.sampling_runtime import sampling_available
...
Provide personalized recommendations based on historical success patterns.""",
    ...
    async def sample(
        ...
        try:
            ...
            # Perform the sampling through the server-side runtime
            # (FastMCP 4 has no ctx.sample; see middleware/sampling_runtime.py)
            if not sampling_available():
                raise ToolError("LLM sampling is not configured on this server")

            try:
                response = await runtime_sample(self.fastmcp_context, **sampling_params)
