...
class ResourceContextManager:
    ...
    def get_relevant_resources_for_task(self, task_description: str) -> List[str]:
        ...
        resources.extend(["user://current/profile", "user://current/email"])

        # Email-related tasks
        if any(
            keyword in task_lower for keyword in ["email", "gmail", "message", "mail"]
        ):
            resources.extend(
                [
        ...
        if any(
            keyword in task_lower for keyword in ["file", "document", "drive", "folder"]
        ):
            resources.extend(
                [
        ...
        if any(
            ...
            for keyword in ["tool", "workflow", "automate", "help"]
        ):
...
class QdrantHistoryEnhancer:
    ...
    async def get_relevant_history(
        self, user_email: str, query: str, limit: int = 3
    ) -> List[Dict[str, Any]]:
        ...
        if not self.qdrant_middleware or not self.qdrant_middleware.is_initialized:
            if self.enable_debug:
                logger.debug(
                    "⚠️ Qdrant middleware not available for history enhancement"
                )
            return []
    ...
    async def enhanced_sample(
        ...
        **kwargs,
        ...
        try:
            if self.enable_debug:
                logger.debug(
                    f"🎯 Enhanced sampling for tool: {self.tool_name}, level: {enhancement_level.value}"
                )
...
"""
        ...
        if enhancement_level in [
            ...
            EnhancementLevel.CONTEXTUAL,
            EnhancementLevel.HISTORICAL,
        ]:
            resource_context = await self._gather_resource_context(
                relevant_resources or []
            )
            if resource_context:
        ...
        if (
            enhancement_level == EnhancementLevel.HISTORICAL
            and include_historical
            and self.history_enhancer
        ):
...
"""
    ...
    async def _gather_resource_context(
        self, relevant_resources: List[str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for resource_uri in relevant_resources:
                resource_data = await self.resource_manager.get_resource_safely(
                    resource_uri
                )
                if resource_data:
    ...
    async def _gather_historical_context(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return await self.history_enhancer.get_relevant_history(
                user_email, query, limit=3
            )
    ...
    def _format_resource_context(self, resource_context: Dict[str, Any]) -> str:
        ...
        if "workspace_activity" in user_context:
            ...
            if isinstance(workspace, dict) and "total_files" in workspace:
                formatted_parts.append(
                    f"- Workspace: {workspace['total_files']} recent files"
                )
        ...
        if "available_tools" in user_context:
            ...
            if isinstance(tools, dict):
                ...
                if tool_count > 0:
                    formatted_parts.append(
                        f"- Tools: {tool_count} enhanced tools available"
                    )
    ...
    def _format_historical_context(self, historical_data: List[Dict[str, Any]]) -> str:
        ...
        for i, pattern in enumerate(historical_data, 1):
            ...
            if success_indicators:
                formatted_parts.append(
                    f"   Success factors: {', '.join(success_indicators)}"
                )
    ...
    async def demo_template_rendering(
        self, template_name: str, user_data: Dict[str, Any] = None, render: bool = False
    ) -> Dict[str, Any]:
...
Provide personalized recommendations based on historical success patterns.""",
    ...
    def _prepare_model_preferences(
        self, preferences: Union[str, List[str], ModelPreferences]
    ) -> Union[str, List[str]]:
...
class EnhancedSamplingMiddleware(Middleware):
    ...
    def __init__(
        ...
        logger.info(
            f"   Qdrant integration: {'enabled' if qdrant_middleware else 'disabled'}"
        )
        logger.info(
    ...
    async def _store_enhanced_context(self, context: MiddlewareContext, tool_name: str):
        ...
        try:
            ...
            resource_manager = ResourceContextManager(
                fastmcp_context=context.fastmcp_context, enable_debug=self.enable_debug
            )
    ...
    async def on_startup(self, mcp: FastMCP):
        ...
        if self.enable_debug:
            logger.debug(
                "🔄 Starting tool transformation for elicitation capabilities..."
            )
        ...
        for tool_name, tool in tools.items():
            ...
            if has_target_tags and tool_name not in self.transformed_tools:
                try:
                    ...
                    if self.enable_debug:
                        logger.debug(
                            f"✅ Transformed tool '{tool_name}' to add elicitation capabilities"
                        )
    ...
    async def _create_elicitation_tool(self, original_tool: Tool) -> Tool:
        ...
        async def elicitation_transform_fn(
            ctx: Annotated[
                Context,
                Field(
                    description="FastMCP context for elicitation support and enhanced LLM sampling"
                ),
            ],
            **kwargs,
        ):
            ...
            try:
                ...
                if "compose" in (original_tool.tags or []):
                    ...
                    if self.enable_debug:
                        logger.debug(
                            f"✨ Enhanced compose tool result for: {original_tool.name}"
                        )
                ...
                logger.error(
                    f"❌ Error in elicitation transform for {original_tool.name}: {e}"
                )
    ...
    async def _create_enhanced_context(self, fastmcp_context: Context, tool_name: str):
        ...
        try:
            # Create resource context manager
            resource_manager = ResourceContextManager(
                fastmcp_context=fastmcp_context, enable_debug=self.enable_debug
            )
...
def create_sampling_context(
    fastmcp_context, tool_name: str, enable_debug: bool = False
) -> SamplingContext:
    """
    ...
    return SamplingContext(
        fastmcp_context=fastmcp_context, tool_name=tool_name, enable_debug=enable_debug
    )
...
class SamplingUtils:
    ...
    async def analyze_sentiment(
        ctx: SamplingContext, text: str, detailed: bool = False
    ) -> Dict[str, Any]:
        """
        Analyze sentiment using LLM sampling.
        ...
        return {"text": text, "sentiment": response.text.strip(), "detailed": detailed}

    @staticmethod
    async def generate_summary(
        ctx: SamplingContext, content: str, max_length: int = 300
    ) -> Dict[str, Any]:
        """
    ...
    async def generate_code(
        ...
        response = await ctx.sample(
            prompt, template=SamplingTemplate.CODE_GENERATION, max_tokens=600
        )
...
"""
            ...
            if demo_results.get("rendered_examples"):
                formatted_output.extend(["", "🎨 **Rendered Examples:**"])
                for name, rendered_data in demo_results["rendered_examples"].items():
                    formatted_output.extend(
                        [
                            f"\n**{name}:**",
                            rendered_data.get(
                                "note", "Rendered content would appear here"
                            ),
                        ]
                    )
