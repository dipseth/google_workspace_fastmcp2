...
class JinjaEnvironmentManager:
    ...
    def setup_jinja2_environment(self) -> Optional[Environment]:
        ...
        if self.templates_dir and self.templates_dir.exists():
            ...
            if self.enable_debug_logging:
                logger.debug(
                    f"📁 Added FileSystemLoader for templates directory: {self.templates_dir}"
                )
        ...
        self.jinja2_env = ContextPreservingEnvironment(
            loader=combined_loader, **env_options
        )
        ...
        if self.enable_debug_logging:
            logger.debug(
                "🎭 Jinja2 environment configured with custom functions, filters, and template macros"
            )
    ...
    def _load_template_macros(self) -> None:
        ...
        if (
            not self.jinja2_env
            or not self.templates_dir
            or not self.templates_dir.exists()
        ):
            return
                ...
                for attr_name in dir(module):
                    if not attr_name.startswith("_"):
                        ...
                        if callable(attr_value):
                            ...
                            if self.enable_debug_logging:
                                logger.debug(
                                    f"📦 Loaded macro '{attr_name}' from {template_name}"
                                )
