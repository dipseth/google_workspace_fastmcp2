...
class TemplateProcessor:
    ...
    async def _preprocess_resource_uris(
        ...
        for match in reversed(matches):
            ...
            try:
                ...
                try:
                    resolved_data = await self.resource_handler.fetch_resource(uri, fastmcp_context)
    ...
    async def _populate_common_resources(self, context: Dict[str, Any], fastmcp_context):
        ...
        try:
            user_data = await self.resource_handler.fetch_resource('user://current/email', fastmcp_context)
            if user_data:
        ...
        try:
            labels_data = await self.resource_handler.fetch_resource('service://gmail/labels', fastmcp_context)
        ...
        try:
            profile_data = await self.resource_handler.fetch_resource('user://current/profile', fastmcp_context)
        ...
        try:
            workspace_data = await self.resource_handler.fetch_resource('recent://all', fastmcp_context)
