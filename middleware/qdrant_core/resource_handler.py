...
class QdrantResourceHandler:
    ...
    def __init__(
        self, client_manager: QdrantClientManager, search_manager: QdrantSearchManager
    ):
        """
    ...
    async def handle_qdrant_resource(self, uri: str, context=None):
        ...
        return self._error_response(
            uri, f"Unknown Qdrant resource type: {resource_type}"
        )
    ...
    async def _handle_collections_list(self, uri: str):
        ...
        try:
            collections = await asyncio.to_thread(
                self.client_manager.client.get_collections
            )
            ...
            for collection in collections.collections:
                try:
                    ...
                    logger.warning(
                        f"Failed to get info for collection {collection.name}: {e}"
                    )
                    collections_info.append(
                        QdrantCollectionInfo(
                            name=collection.name,
        ...
        try:
            ...
            if collection_name in collection_names:
                collection_info = await asyncio.to_thread(
                    self.client_manager.client.get_collection, collection_name
                )
    ...
    async def _handle_collection_responses(self, uri: str, collection_name: str):
        ...
        try:
            ...
            for point in points[:20]:  # Limit to 20 most recent
                ...
                if payload.get("compressed", False) and payload.get("compressed_data"):
                    try:
                        ...
                        logger.warning(
                            f"Failed to decompress data for point {point.id}: {e}"
                        )
            ...
            return self._error_response(
                uri, f"Failed to get collection responses: {str(e)}"
            )

    async def _handle_point_details(
        self, uri: str, collection_name: str, point_id: str
    ):
        """
        ...
        try:
            ...
            try:
                ...
                parsed_payload = self.client_manager.parse_tool_response_payload(
                    payload
                )
                ...
                if compressed and parsed_payload.get("compressed_data"):
                    try:
                        decompressed = self.client_manager._decompress_data(
                            parsed_payload["compressed_data"]
                        )
                        try:
                        ...
                        logger.warning(
                            f"Failed to decompress data for point {point_id}: {e}"
                        )
                        response_data = {
                ...
                nearby_points = await self._find_nearby_points(
                    collection_name, timestamp, session_id, str(point.id)
                )
            ...
            return self._error_response(
                uri, f"Failed to retrieve point details: {str(e)}"
            )

    async def _handle_collection_search(
        self, uri: str, collection_name: str, query: str
    ):
        """
...
async def handle_qdrant_resource_legacy(middleware, uri: str, context=None):
    ...
    if hasattr(middleware, "client_manager") and hasattr(middleware, "search_manager"):
        logger.warning(
            "Using fallback resource handler creation - consider updating middleware integration"
        )
        resource_handler = QdrantResourceHandler(
            middleware.client_manager, middleware.search_manager
        )
