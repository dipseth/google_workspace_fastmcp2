...
|-------|------|----------|---------|-------------|
| `id` | Union[int[int], str[str], UUID] | Yes | — | Search result |
| `version` | int | Yes | — | Point version |
| `score` | float | Yes | — | Points vector distance to the query vector |
...
| `order_value` | Union[int[int], float[float], NoneType] | No | — | Order-by value |
