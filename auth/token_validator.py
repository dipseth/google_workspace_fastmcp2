...
def validate_google_token_with_access_control(token: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if "error" in token_info:
            logger.warning(f"❌ Token validation error: {token_info.get('error_description', 'Unknown')}")
            return None
        ...
        if not validate_user_access(email):
            logger.warning(f"🚫 Token validation failed: User {email} not authorized (no stored credentials)")
            return None
...
class AccessControlBearerAuthProvider:
    ...
    def __init__(self, jwks_uri: str, issuer: str, audience: Optional[str] = None,
        ...
        logger.info(f"  Access control: Enforces stored credentials")
