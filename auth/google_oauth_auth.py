...
class GoogleProviderSettings(BaseModel):
    ...
    base_url: str = Field(
        default_factory=lambda: settings.base_url, description="Server base URL"
    )
...
def create_modern_google_provider(
    ...
    try:
        logger.info(
            "🔑 Creating modern FastMCP 2.12.x GoogleProvider with ScopeRegistry integration..."
        )
        ...
        logger.info(
            "✅ Modern GoogleProvider configured with ScopeRegistry integration"
        )
        logger.info("  🏗️  OAuth Proxy Architecture: Enhanced security")
        logger.info("  🔐 PKCE Support: Built-in")
        ...
        logger.info(
            f"  🔄 Redirect URI: {provider_settings.redirect_uri} (existing configuration)"
        )
...
def setup_google_oauth_auth(
    enable_modern_provider: bool = True, scope_group: str = "base"
    ...
    if enable_modern_provider:
        modern_provider = create_modern_google_provider(
            enable_unified_auth=True, auth_type="GOOGLE", scope_group=scope_group
        )
        if modern_provider:
...
def get_google_oauth_metadata() -> Dict[str, Any]:
    ...
    return {
        "issuer": "https://accounts.google.com",
        "authorization_endpoint": oauth_config.get(
            "auth_uri", "https://accounts.google.com/o/oauth2/auth"
        ),
        "token_endpoint": oauth_config.get(
...
def generate_user_token(
    user_email: str, scopes: Optional[list[str]] = None, expires_in_seconds: int = 3600
) -> str:
