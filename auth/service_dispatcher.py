...
class ServiceDispatchResult:
    ...
    def __init__(self, service: Any, service_name: str, confidence_score: float, 
                 routing_decision: ServiceRoutingDecision, metadata: Dict[str, Any] = None):
        """
...
class ServiceDispatchError(Exception):
    ...
    def __init__(self, message: str, suggested_services: List[str] = None, 
...
class EnhancedServiceDispatcher:
    ...
    async def dispatch_service(self, request: Dict[str, Any], user_email: str, 
                              available_scopes: List[str] = None) -> ServiceDispatchResult:
        """
        ...
        validation = self.service_router.validate_routing_decision(routing_decision, available_scopes)
        
        if not validation['is_valid']:
            # Try fallback options
            if routing_decision.fallback_available:
                self.logger.warning(f"Primary routing failed, trying alternatives")
                return await self._try_fallback_routing(routing_decision, user_email, available_scopes, request)
            else:
                # No alternatives available
                available_services = [svc['name'] for svc in self.service_router.get_available_services()]
                raise ServiceDispatchError(
                    f"Cannot dispatch request: {'; '.join(validation['warnings'])}",
                    suggested_services=[alt.service for alt in routing_decision.alternatives],
        ...
        try:
            ...
            result = ServiceDispatchResult(
                ...
                metadata={
                    ...
                    'service_metadata': self._get_service_registry_info(primary_service)
                }
            )
            
            self.logger.info(f"Successfully dispatched to {primary_service} (confidence: {result.confidence_score:.2f})")
            return result
            
        except Exception as e:
            self.logger.error(f"Failed to get service instance for {primary_service}: {e}")
    ...
    async def _try_fallback_routing(self, routing_decision: ServiceRoutingDecision, 
                                   user_email: str, available_scopes: List[str], 
                                   request: Dict[str, Any]) -> ServiceDispatchResult:
            ...
            try:
                ...
                result = ServiceDispatchResult(
                    ...
                    metadata={
                        ...
                        'service_metadata': self._get_service_registry_info(alternative.service)
                    }
                )
    ...
    async def _get_service_instance(self, service_name: str, user_email: str, 
                                   available_scopes: List[str] = None) -> Any:
        """
    ...
    def clear_service_cache(self, user_email: str = None):
        ...
        if user_email:
            ...
            self.logger.info(f"Cleared {len(keys_to_remove)} cached services for {user_email}")
        else:
    ...
    def get_service_suggestions(self, partial_name: str) -> List[Dict[str, Any]]:
        ...
        for service in available_services:
            ...
            elif any(partial_lower in op.lower() for op in service.get('operations', [])):
                suggestions.append({
                    'service': service,
                    'match_type': 'operations',
                    'relevance': 0.6
                })
...
def generate_enhanced_error_message(error: ServiceDispatchError, request: Dict[str, Any]) -> str:
    """
    ...
    """
    message = f"**Service Routing Error**\n\n"
