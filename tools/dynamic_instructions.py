...
def _build_services_section() -> str:
    ...
    for service_name in ScopeRegistry.get_all_services():
        ...
        if metadata:
            ...
            lines.append(f"- **{metadata.icon} {metadata.name}**: {metadata.description}")
...
class DynamicInstructionsBuilder:
    ...
    def is_qdrant_available(self) -> bool:
        ...
        try:
            return (
                hasattr(self.qdrant_middleware, 'client_manager') and
                self.qdrant_middleware.client_manager.is_available
            )
        except Exception:
            return False
    ...
    async def get_tool_analytics(self) -> Optional[Dict]:
        ...
        try:
            ...
            if analytics and not analytics.get("error"):
                logger.info(f"📊 Retrieved analytics: {analytics.get('total_responses', 0)} total responses")
                return analytics
            else:
    ...
    def _format_qdrant_status(self) -> str:
        ...
        if self.is_qdrant_available:
            collection = settings.tool_collection if hasattr(settings, 'tool_collection') else "mcp_tool_responses"
...
def get_instructions_builder(qdrant_middleware: Optional[Any] = None) -> DynamicInstructionsBuilder:
    """
    ...
    if _builder is None or (qdrant_middleware and _builder.qdrant_middleware != qdrant_middleware):
...
async def update_mcp_instructions(mcp: Any, qdrant_middleware: Optional[Any] = None) -> bool:
    """
...
async def refresh_instructions(mcp: Any) -> bool:
    ...
    try:
        if _builder:
            ...
            dynamic_instructions = await _builder.build_dynamic_instructions(force_refresh=True)
            mcp.instructions = dynamic_instructions
