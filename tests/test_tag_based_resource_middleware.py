...
class TestTagBasedResourceMiddleware:
    ...
    def test_service_uri_pattern_matching(self, middleware):
        ...
        valid_cases = [
            ...
            ("service://calendar/events/event_123", ("calendar", "events", "event_123")),
    ...
    @pytest.mark.asyncio
    async def test_middleware_passes_through_non_service_uris(self, middleware, mock_context, mock_call_next):
    ...
    @pytest.mark.asyncio
    async def test_middleware_intercepts_service_uris(self, middleware, mock_context, mock_call_next):
    ...
    @pytest.mark.asyncio
    async def test_invalid_service_uri_format_error(self, middleware, mock_context, mock_call_next):
    ...
    @pytest.mark.asyncio
    async def test_handle_service_lists_gmail(self, middleware, mock_context, sample_tools):
    ...
    @pytest.mark.asyncio
    async def test_handle_service_lists_unsupported_service(self, middleware, mock_context):
        """Test service lists endpoint for unsupported service."""
        result = await middleware._handle_service_lists("unsupported_service", mock_context)
    ...
    @pytest.mark.asyncio
    async def test_handle_service_lists_filters_by_available_tools(self, middleware, mock_context):
    ...
    async def test_handle_list_items_success(self, mock_get_user_email, middleware, mock_context, sample_tools):
    ...
    @pytest.mark.asyncio
    @patch('middleware.tag_based_resource_middleware.get_user_email_context')
    async def test_handle_list_items_no_user_email(self, mock_get_user_email, middleware, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_handle_list_items_unsupported_service(self, middleware, mock_context):
        """Test list items for unsupported service."""
        result = await middleware._handle_list_items("unsupported", "filters", mock_context)
    ...
    @pytest.mark.asyncio
    async def test_handle_list_items_unsupported_list_type(self, middleware, mock_context):
        """Test list items for unsupported list type."""
        result = await middleware._handle_list_items("gmail", "unsupported_list", mock_context)
        
        content = json.loads(result["contents"][0]["text"])
        assert content["error"] is True
        assert "Unsupported list type 'unsupported_list' for service 'gmail'" in content["message"]
    ...
    async def test_handle_list_items_no_list_tool(self, middleware, mock_context):
        ...
        result = await middleware._handle_list_items("slides", "presentations", mock_context)
    ...
    @pytest.mark.asyncio
    @patch('middleware.tag_based_resource_middleware.get_user_email_context')
    async def test_handle_specific_item_success(self, mock_get_user_email, middleware, mock_context, sample_tools):
        ...
        get_tool.call.return_value = {"filter_id": "123", "criteria": {"from": "test@test.com"}}
        
        result = await middleware._handle_specific_item("gmail", "filters", "123", mock_context)
        
        # Verify tool was called with correct parameters
        get_tool.call.assert_called_once_with(user_google_email="test@example.com", filter_id="123")
    ...
    async def test_handle_specific_item_no_get_tool(self, middleware, mock_context):
        ...
        result = await middleware._handle_specific_item("gmail", "labels", "123", mock_context)
    ...
    @pytest.mark.asyncio
    async def test_get_available_tools_success(self, middleware, mock_context, sample_tools):
    ...
    @pytest.mark.asyncio
    async def test_call_tool_with_context_success(self, middleware, mock_context, sample_tools):
    ...
    @pytest.mark.asyncio
    async def test_call_tool_with_context_tool_not_found(self, middleware, mock_context, sample_tools):
        """Test tool calling when tool not found."""
        mock_context.fastmcp_context = MockFastMCPContext(sample_tools)
        
        with pytest.raises(RuntimeError, match="Tool 'nonexistent_tool' not found"):
            await middleware._call_tool_with_context(mock_context, "nonexistent_tool", {})
    
    @pytest.mark.asyncio
    async def test_call_tool_with_context_tool_call_fails(self, middleware, mock_context, sample_tools):
        ...
        with pytest.raises(Exception, match="Tool failed"):
            await middleware._call_tool_with_context(mock_context, "list_gmail_filters", {})
    ...
    def test_service_metadata_completeness(self, middleware):
        ...
        expected_services = ["gmail", "drive", "calendar", "docs", "sheets", "chat", "forms", "slides", "photos"]
    ...
    def test_service_metadata_list_types_structure(self, middleware):
        ...
        for service_name, service_meta in middleware.SERVICE_METADATA.items():
            ...
            for list_type_name, list_type_info in list_types.items():
                ...
                assert has_list_tool or has_get_tool, f"No tools for {service_name}/{list_type_name}"
    ...
    @pytest.mark.asyncio
    @patch('middleware.tag_based_resource_middleware.get_user_email_context')
    async def test_full_workflow_service_lists(self, mock_get_user_email, middleware, mock_call_next, sample_tools):
    ...
    @pytest.mark.asyncio
    @patch('middleware.tag_based_resource_middleware.get_user_email_context')
    async def test_full_workflow_list_items(self, mock_get_user_email, middleware, mock_call_next, sample_tools):
    ...
    @pytest.mark.asyncio
    @patch('middleware.tag_based_resource_middleware.get_user_email_context')
    async def test_full_workflow_specific_item(self, mock_get_user_email, middleware, mock_call_next, sample_tools):
    ...
    @pytest.mark.asyncio
    async def test_exception_handling_in_main_method(self, middleware, mock_context, mock_call_next):
        ...
        with patch.object(middleware, '_handle_service_lists', side_effect=Exception("Test error")):
            result = await middleware.on_read_resource(mock_context, mock_call_next)
    ...
    @pytest.mark.asyncio
    async def test_service_root_access_not_implemented(self, middleware, mock_context, mock_call_next):
