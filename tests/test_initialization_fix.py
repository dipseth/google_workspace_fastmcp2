...
class TestQdrantInitializationFlow:
    ...
    def mock_middleware(self):
        ...
        with (
            patch(
                "middleware.qdrant_middleware.QdrantClientManager"
            ) as mock_client_manager_class,
            patch(
                "middleware.qdrant_middleware.QdrantStorageManager"
            ) as mock_storage_manager_class,
            patch(
                "middleware.qdrant_middleware.QdrantSearchManager"
            ) as mock_search_manager_class,
            patch(
            ...
            middleware = QdrantUnifiedMiddleware(
                qdrant_host="localhost", qdrant_port=6333, enabled=True
            )
    ...
    @pytest.mark.asyncio
    async def test_initialize_middleware_and_reindexing_calls_background_reindexing(
        self, mock_middleware
    ):
        """Test that the new initialization method properly starts background reindexing."""
        with patch.object(
            mock_middleware, "_start_background_reindexing", new_callable=AsyncMock
        ) as mock_start_reindexing:
    ...
    async def test_backward_compatibility_initialize_method(self, mock_middleware):
        ...
        with patch.object(
            mock_middleware,
            "initialize_middleware_and_reindexing",
    ...
    async def test_on_call_tool_uses_correct_initialization(self, mock_middleware):
        ...
        with patch.object(
            ...
            with (
                patch(
                    "middleware.qdrant_middleware.get_session_context",
                    return_value="test_session",
                ),
                patch.object(
                    mock_middleware.storage_manager,
                    "_store_response_with_params",
                    new_callable=AsyncMock,
                ),
            ):
                ...
                # Call on_call_tool
                result = await mock_middleware.on_call_tool(
                    mock_context, mock_call_next
                )
