...
pytestmark = pytest.mark.skipif(
    ...
    reason="Qdrant server is not running or not available"
)
...
class TestCardFrameworkWrapper:
    ...
    def setup_class(cls):
        ...
        try:
            ...
            cls._shared_wrapper = ModuleWrapper(
                ...
                exclude_modules=["numpy", "pandas", "matplotlib", "scipy", "sklearn",
                                "tensorflow", "torch", "keras", "django", "flask",
                                "requests", "bs4", "selenium", "sqlalchemy", "pytest",
                                "unittest", "nose", "marshmallow", "dataclasses_json",
                                "stringcase", "qdrant_client", "sentence_transformers"]
            )
            logger.info(f"Created shared ModuleWrapper with collection: {cls._collection_name}")
        except Exception as e:
    ...
    def teardown_class(cls):
        ...
        if cls._shared_wrapper:
            try:
                ...
                logger.warning(f"Failed to delete collection {cls._collection_name}: {e}")
    ...
    def test_card_framework_initialization(self, card_framework_wrapper):
        ...
        logger.info(f"card_framework ModuleWrapper initialized with {len(card_framework_wrapper.components)} components")
        ...
        for component_name in key_components:
            ...
            for result in results:
                if component_name.lower() in result['name'].lower():
                    found = True
                    logger.info(f"Found component {component_name}: {result['path']} (score: {result['score']:.4f})")
                    break
            
            if not found:
                logger.warning(f"Component {component_name} not found in search results")
    ...
    def test_create_and_send_card(self, card_framework_wrapper):
        ...
        try:
            ...
            card_results = card_framework_wrapper.search("Card", limit=10, score_threshold=0.3)
            logger.info(f"Card search returned {len(card_results)} results")
            for i, result in enumerate(card_results):
                logger.info(f"Card result {i+1}: {result['name']} ({result['path']}) - Score: {result['score']:.4f}")
                if result['component'] is not None and (card_component is None or result['score'] > 0.5):
                    card_component = result['component']
                    logger.info(f"Selected Card component: {result['path']}")
            
            section_results = card_framework_wrapper.search("Section", limit=10, score_threshold=0.3)
            logger.info(f"Section search returned {len(section_results)} results")
            for i, result in enumerate(section_results):
                logger.info(f"Section result {i+1}: {result['name']} ({result['path']}) - Score: {result['score']:.4f}")
                if result['component'] is not None and (section_component is None or result['score'] > 0.5):
                    section_component = result['component']
                    logger.info(f"Selected Section component: {result['path']}")
            
            text_results = card_framework_wrapper.search("TextParagraph", limit=10, score_threshold=0.3)
            logger.info(f"TextParagraph search returned {len(text_results)} results")
            for i, result in enumerate(text_results):
                logger.info(f"TextParagraph result {i+1}: {result['name']} ({result['path']}) - Score: {result['score']:.4f}")
                if result['component'] is not None and (text_component is None or result['score'] > 0.5):
                    text_component = result['component']
                    logger.info(f"Selected TextParagraph component: {result['path']}")
            
            button_results = card_framework_wrapper.search("Button", limit=10, score_threshold=0.3)
            logger.info(f"Button search returned {len(button_results)} results")
            for i, result in enumerate(button_results):
                logger.info(f"Button result {i+1}: {result['name']} ({result['path']}) - Score: {result['score']:.4f}")
                if result['component'] is not None and (button_component is None or result['score'] > 0.5):
            ...
            for i, component_path in enumerate(sorted(card_framework_wrapper.components.keys())[:50]):  # Limit to first 50
            ...
            if missing_components:
                ...
                pytest.skip(f"Could not find required components: {', '.join(missing_components)}")
            ...
            section = self._create_component_instance(
                ...
                {
                    "header": "Test Section",
                    "widgets": [text_paragraph, button] if text_paragraph and button else []
                }
            )
            ...
            # Check response
            assert response.status_code == 200, f"Failed to send card: {response.status_code} - {response.text}"
            ...
            logger.info("For comparison, creating a card directly using Google Chat API format...")
            ...
            assert direct_response.status_code == 200, f"Failed to send direct card: {direct_response.status_code} - {direct_response.text}"
            logger.info(f"Direct card sent successfully! Status: {direct_response.status_code}")
    ...
    def test_create_and_send_complex_card(self, card_framework_wrapper):
        ...
        try:
            ...
            # Check response
            assert response.status_code == 200, f"Failed to send complex card: {response.status_code} - {response.text}"
            logger.info(f"Complex card sent successfully! Status: {response.status_code}")
