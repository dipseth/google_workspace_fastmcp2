...
TEST_EMAIL = os.getenv("TEST_EMAIL_ADDRESS", "sethrivers@gmail.com")


class TestEnhancedGmailFilters:
    """Test enhanced Gmail filter functionality through MCP server."""
    ...
    async def _test_list_gmail_filters_no_auth(self, client):
        ...
        assert ("authentication" in content.lower() or
                ...
                "found" in content.lower() and "filters" in content.lower())
    ...
    async def test_enhanced_retroactive_filter_creation(self, client):
        ...
        if ("authentication" not in content.lower() and
            ...
            assert "filter created successfully" in content.lower()
            # Should mention retroactive application
            assert ("applied" in content.lower() or "retroactive" in content.lower() or
                    "existing" in content.lower() or "messages" in content.lower())
        else:
    ...
    async def test_missing_required_parameters(self, client):
        ...
        # Test create_gmail_filter without user_google_email
        with pytest.raises(Exception) as exc_info:
            await client.call_tool("create_gmail_filter", {
                "from_address": "@test.com",
                "add_label_ids": ["Label_1"]
            })
        assert "required" in str(exc_info.value).lower() or "missing" in str(exc_info.value).lower()
        
        # Test get_gmail_filter without filter_id
        with pytest.raises(Exception) as exc_info:
            await client.call_tool("get_gmail_filter", {
                "user_google_email": TEST_EMAIL
            })
        assert "required" in str(exc_info.value).lower() or "missing" in str(exc_info.value).lower()
        
        # Test delete_gmail_filter without filter_id
        with pytest.raises(Exception) as exc_info:
            await client.call_tool("delete_gmail_filter", {
                "user_google_email": TEST_EMAIL
            })
        assert "required" in str(exc_info.value).lower() or "missing" in str(exc_info.value).lower()
...
class TestStarbucksFilterScenario:
    ...
    async def test_starbucks_filter_creation_comprehensive(self, client):
        ...
        if ("authentication" in content.lower() or
            ...
            if "filter created successfully" in content.lower():
                assert "filter created successfully" in content.lower()
                # Should mention the specific label (updated to valid label)
                assert "STARRED" in content
            else:
                # Accept user-friendly error messages as valid test outcomes
    ...
    async def test_list_filters_after_creation(self, client):
        ...
        if "authentication error: please check your gmail permissions" in content.lower():
            ...
            # Should list filters if authenticated
            assert ("found" in content.lower() and "filters" in content.lower()) or "no gmail filters" in content.lower()
        else:
