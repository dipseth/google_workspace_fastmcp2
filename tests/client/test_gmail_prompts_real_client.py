...
"""
    ...
    async def test_list_prompts(self, client):
        ...
        gmail_prompts_found = [name for name in expected_gmail_prompts if name in prompt_names]
        ...
        if gmail_prompts_found:
            ...
            print("⚠️  No Gmail prompts found - might need to register them in server startup")
    ...
    async def test_prompt_filtering_by_tags(self, client):
        ...
        if gmail_tagged_prompts:
            assert "quick_email_demo" in simple_tagged_prompts or len(simple_tagged_prompts) > 0
    ...
    async def test_prompt_with_resource_context(self, client):
        ...
        has_resource_evidence = (
            "service://gmail/labels" in content or
            "user://current/email" in content or
            "Authentication required" in content or
            "No authenticated user found" in content or
            "label" in content.lower() and ("id" in content or "name" in content)
        )
        
        assert has_resource_evidence, f"No evidence of resource access in content: {content[:500]}"
