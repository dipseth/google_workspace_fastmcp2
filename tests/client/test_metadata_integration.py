...
class TestMetadataIntegration:
        ...
        for tool in sample_tools:
            ...
            if hasattr(tool.inputSchema, '__dict__'):
                ...
                if 'properties' in schema:
                    assert isinstance(schema['properties'], dict), f"Tool {tool.name} properties should be dict"
    ...
    async def test_metadata_consistency(self, client):
        ...
        print(f"✅ Metadata is consistent across calls")
    ...
    async def test_metadata_descriptions(self, client):
        ...
        print(f"✅ All sampled tools have meaningful descriptions")
    ...
    async def test_metadata_version_info(self, client):
        ...
        for tool in tools:
            ...
            elif 'v' in tool.description.lower() or 'version' in tool.description.lower():
                tools_with_version += 1
        
        print(f"✅ Metadata version check complete")
    ...
    async def test_metadata_error_schemas(self, client):
        ...
        try:
            ...
            assert any(word in error_msg.lower() for word in ['validation', 'parameter', 'invalid', 'required']), \
                "Error should be descriptive"
        
        print(f"✅ Error metadata properly structured")
...
class TestMetadataSourceOfTruth:
    ...
    async def test_metadata_driven_validation(self, client):
        ...
        for test in test_cases:
            try:
                ...
                if test['expect_error']:
                    ...
                    assert any(word in content.lower() for word in ['error', 'invalid', 'failed']), \
                        f"Should indicate error for invalid params"
            except Exception as e:
        ...
        print(f"✅ Validation is metadata-driven")
    ...
    async def test_metadata_backwards_compatibility(self, client):
        ...
        print(f"✅ Metadata maintains backwards compatibility")
    ...
    async def test_metadata_extensibility(self, client):
        ...
        print(f"✅ Metadata schema is extensible")
        print(f"   Tools with extensions: {tools_with_extensions}/{len(tools)}")
        print(f"   Resources with extensions: {resources_with_extensions}/{len(resources)}")
