...
class TestMCPServer:
    ...
    async def test_start_google_auth(self, client):
        ...
        assert ("https://accounts.google.com/o/oauth2/auth" in content or
                "authUrl" in content or
                "https://localhost" in content)  # Local auth redirect
    ...
    async def test_read_tools_list_resource(self, client):
        ...
        expected_categories = [
            "drive_tools", "gmail_tools", "docs_tools", "forms_tools",
            "calendar_tools", "chat_tools", "auth_tools"
        ]
    ...
    async def test_read_detailed_tools_resource(self, client):
        ...
        assert "detailed_tools" in detailed_data or "tools" in detailed_data or "total_tools" in detailed_data
...
class TestGmailTools:
    ...
    async def _test_list_gmail_labels_no_auth(self, client):
        ...
        assert ("authentication" in content.lower() or
                ...
                "not authenticated" in content.lower() or
                "labels" in content.lower() or  # Success response with labels
                '"error":null' in content.lower())  # Successful response indicator
...
class TestDocsTools:
    ...
    @pytest.mark.asyncio
    async def test_docs_tools_available(self, client, real_drive_document_id, real_drive_folder_id):
    ...
    async def _test_get_doc_content_no_auth(self, client, real_drive_document_id=None):
        ...
        assert ("authentication" in content.lower() or
                "credentials" in content.lower() or
                "not authenticated" in content.lower() or
                "document not found" in content.lower() or
                "permission denied" in content.lower() or
                "content" in content.lower())  # Success case returns document content
    ...
    async def _test_create_doc_no_auth(self, client):
        ...
        assert ("authentication" in content.lower() or
                "credentials" in content.lower() or
                "not authenticated" in content.lower() or
                "created google doc" in content.lower() or
                "created empty google doc" in content.lower() or
                '"success":true' in content.lower())  # Success indicator
...
class TestGmailLabelColors:
    ...
    async def _test_create_label_with_colors_no_auth(self, client):
        ...
        assert ("authentication" in content.lower() or
                "credentials" in content.lower() or
                "not authenticated" in content.lower() or
                "label created successfully" in content.lower() or
                '"success":true' in content.lower() or
                "conflict" in content.lower() or  # Label already exists
                "already exist" in content.lower())
    ...
    async def _test_update_label_with_colors_no_auth(self, client):
        ...
        assert ("authentication" in content.lower() or
                ...
                "label id is required" in content.lower() or
                "not found" in content.lower() or  # Label doesn't exist
    ...
    async def _test_invalid_color_validation(self, client):
        ...
        assert ("invalid text color" in content.lower() or
                "adjusted" in content.lower() or  # Color was auto-corrected
                "success" in content.lower())  # Operation succeeded with correction
        ...
        # Server may either reject invalid colors or auto-correct them
        assert ("invalid background color" in content.lower() or
                "adjusted" in content.lower() or  # Color was auto-corrected
                "success" in content.lower())  # Operation succeeded with correction
    ...
    async def test_missing_required_params(self, client):
        ...
        try:
            ...
            if result and result.content:
                ...
                assert ("required" in content or
                        "missing" in content or
                        "document_id" in content)
        except Exception as exc_info:
            # Should indicate missing parameter
            assert ("required" in str(exc_info).lower() or
                    "missing" in str(exc_info).lower() or
                    "document_id" in str(exc_info).lower())
