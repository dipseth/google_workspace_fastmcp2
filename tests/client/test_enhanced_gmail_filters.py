...
class TestEnhancedGmailFilters:
    ...
    async def test_gmail_filter_tools_available(self, client):
        ...
        filter_tools_available = [
            tool for tool in expected_filter_tools if tool in tool_names
        ]
    ...
    async def _test_create_gmail_filter_no_auth(self, client):
        ...
        assert (
            ...
            or "credentials" in content.lower()
            or "not authenticated" in content.lower()
            or "filter created successfully" in content.lower()
            or "filter already exists" in content.lower()
            or "please check your gmail permissions" in content.lower()
            or "invalid label" in content.lower()
        )
    ...
    async def _test_get_gmail_filter_no_auth(self, client, real_gmail_filter_id=None):
        ...
        assert (
            ...
            or "credentials" in content.lower()
            or "not authenticated" in content.lower()
            or "filter details" in content.lower()
            or "filter not found" in content.lower()
        )

    async def _test_delete_gmail_filter_no_auth(
        self, client, real_gmail_filter_id=None
    ):
        ...
        assert (
            ...
            or "credentials" in content.lower()
            or "not authenticated" in content.lower()
            or "filter deleted successfully" in content.lower()
            or "filter not found" in content.lower()
        )
    ...
    async def test_enhanced_retroactive_filter_creation(self, client):
        ...
        if (
            ...
            and "please check your gmail permissions" not in content.lower()
            and "filter already exists" not in content.lower()
        ):
            ...
            assert (
                "applied" in content.lower()
                or "retroactive" in content.lower()
                or "existing" in content.lower()
                or "messages" in content.lower()
                or "processed" in content.lower()
            )
        else:
    ...
    async def test_enhanced_filter_features_parameters(self, client):
        ...
        if (
            ...
            assert (
                "validation" not in content.lower() or "successfully" in content.lower()
            )
        else:
            # Authentication error expected
            assert (
                "authentication" in content.lower() or "credentials" in content.lower()
            )
    ...
    async def test_missing_required_parameters(self, client):
        ...
        try:
            result = await client.call_tool(
                "create_gmail_filter",
                {"from_address": "@test.com", "add_label_ids": ["Label_1"]},
            )
            assert result is not None and result.content
            content = result.content[0].text
            assert any(
                keyword in content.lower()
                for keyword in ["required", "missing", "error", "invalid"]
            )
        ...
        try:
            result = await client.call_tool(
                "get_gmail_filter", {"user_google_email": TEST_EMAIL}
            )
            assert result is not None and result.content
            content = result.content[0].text
            assert any(
        ...
        try:
            result = await client.call_tool(
                "delete_gmail_filter", {"user_google_email": TEST_EMAIL}
            )
            assert result is not None and result.content
            content = result.content[0].text
            assert any(
    ...
    async def test_filter_creation_response_structure(self, client):
        ...
        result = await client.call_tool(
            "create_gmail_filter",
            {
                ...
                "add_label_ids": [
                    "INBOX"
                ],  # Use valid system label instead of "Label_Test"
            },
        )
        ...
        if (
            ...
            and "invalid label" not in content.lower()
        ):
...
class TestStarbucksFilterScenario:
    ...
    async def test_starbucks_filter_creation_comprehensive(self, client):
        ...
        result = await client.call_tool(
            "create_gmail_filter",
            {
                ...
                "add_label_ids": [
                    "STARRED"
                ],  # Use valid system label instead of "Label_28"
            },
        )
        ...
        if (
            ...
            enhanced_features = {
                ...
                "retroactive_mentioned": any(
                    word in content.lower()
                    for word in ["retroactive", "existing", "applied"]
                ),
                "message_processing": any(
                    word in content.lower()
                ...
                "performance_metrics": any(
                    word in content.lower() for word in ["time", "rate", "performance"]
                ),
            }
    ...
    async def test_list_filters_after_creation(self, client):
        ...
        if (
            "authentication error: please check your gmail permissions"
            in content.lower()
        ):
            ...
            assert (
                ...
                and ("count" in content.lower() or "id" in content.lower())
            ) or "no gmail filters" in content.lower()
        else:
