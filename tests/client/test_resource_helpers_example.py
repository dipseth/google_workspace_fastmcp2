...
class TestResourceHelpersExample:
    ...
    async def test_resource_id_fetcher_basic_usage(self, client):
        ...
        all_ids = [gmail_message_id, gmail_filter_id, drive_document_id, calendar_event_id]
        print(f"✅ Resource helper test completed. Found {sum(1 for id in all_ids if id is not None)} real IDs")

    @pytest.mark.asyncio
    async def test_using_real_gmail_message_id_in_test(self, client, real_gmail_message_id):
        ...
        assert real_gmail_message_id is not None, "Should have a real message ID or fallback"
        ...
        print(f"✅ Test using real message ID completed")

    @pytest.mark.asyncio
    async def test_using_real_drive_document_id_in_test(self, client, real_drive_document_id):
        ...
        assert real_drive_document_id is not None, "Should have a real document ID or fallback"
        print(f"✅ Test using real Drive document ID completed")

    @pytest.mark.asyncio
    async def test_using_real_calendar_event_id_in_test(self, client, real_calendar_event_id):
        ...
        assert real_calendar_event_id is not None, "Should have a real event ID or fallback"
        print(f"✅ Test using real Calendar event ID completed")
    ...
    async def test_generic_service_resource_access(self, client):
        ...
        print(f"✅ Successfully fetched IDs from {successful_fetches}/{len(services_to_test)} service resources")
