...
class TestUserResources:
    ...
    async def test_read_current_user_email_resource(self, client):
        ...
        try:
            ...
            logger.info(
                f"Reading user email resource failed (expected without full auth): {e}"
            )
    ...
    async def test_read_current_user_profile_resource(self, client):
        ...
        try:
            ...
            # May fail if not authenticated properly, but that's okay for testing
            logger.info(
                f"Reading user profile resource failed (expected without full auth): {e}"
            )
    ...
    async def test_read_auth_session_current_resource(self, client):
        ...
        try:
            ...
            if hasattr(first_content, "text"):
                ...
                logger.info(
                    f"Successfully read auth session: {session.get('status', 'unknown')}"
                )

        except Exception as e:
            # May fail if not authenticated properly, but that's okay for testing
            logger.info(
                f"Reading auth session resource failed (expected without full auth): {e}"
            )
    ...
    async def test_read_template_user_email_resource(self, client):
        ...
        try:
            ...
            # May fail if not authenticated properly, but that's okay for testing
            logger.info(
                f"Reading template resource failed (expected without full auth): {e}"
            )
    ...
    async def test_read_user_profile_by_email_template(self, client):
        ...
        try:
            ...
            # May fail if not authenticated properly, but that's okay for testing
            logger.info(
                f"Reading user profile template failed (expected without full auth): {e}"
            )
    ...
    async def test_filter_resources_by_tags(self, client):
        ...
        # Log summary
        logger.info(
            f"Found {len(auth_resources)} auth-related resources and {len(tagged_resources)} tagged resources"
        )
...
class TestEnhancedTools:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="Enhanced tools (list_my_drive_files, search_my_gmail, etc.) were planned but not implemented"
    )
    ...
    async def test_list_my_drive_files_tool(self, client):
        ...
        try:
            ...
            result = await client.call_tool(
                "list_my_drive_files", {"query": "test", "page_size": 5}
            )
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="Enhanced tools (list_my_drive_files) were planned but not implemented"
    )
    async def test_enhanced_tool_parameter_reduction(self, client):
        ...
        if legacy_drive_tool:
            ...
            logger.info(
                f"✅ Enhanced tool has {enhanced_params} params vs legacy {legacy_params} params"
            )
        else:
...
class TestResourceTemplateIntegration:
    ...
    async def test_multi_resource_tool_integration(self, client):
        ...
        try:
            ...
            if "authenticated" in content.lower():
                logger.info(
                    "✅ Multi-resource tool successfully combined resource data"
                )
            else:
    ...
    async def test_raw_mcp_protocol_access(self, client):
        ...
        logger.info(
            f"Raw MCP: Found {len(templates_result.resourceTemplates)} templates"
        )
