...
class TestFormatSheetRangeValidation:
    ...
    async def test_number_format_validation_fixed(self, client, test_spreadsheet_id):
        ...
        if (
            ...
            and "no valid credentials" not in content.lower()
        ):
            validation_indicators = [
                (
                    "both" in content.lower() and "together" in content.lower()
                ),  # Our validation
                (
                    "both" in content.lower() and "must both be set" in content.lower()
                ),  # API validation
                (
    ...
    async def test_format_sheet_range_response_structure(
        ...
        if "successfully applied" in content.lower():
            ...
            assert any(
                indicator in content_lower for indicator in structure_indicators
            ), f"Response should contain structure indicators: {content}"

    @pytest.mark.asyncio
    async def test_parameter_validation_comprehensive(
        self, client, test_spreadsheet_id
    ):
...
class TestFormatSheetRangeIntegration:
    ...
    @pytest.mark.asyncio
    async def test_format_sheet_range_with_data_operations(
        self, client, test_spreadsheet_id
    ):
