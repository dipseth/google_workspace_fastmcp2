...
class TestRoutingImprovements:
    ...
    async def test_routing_confidence_scores(self, client):
        ...
        test_result = await client.call_tool("list_gmail_labels", {
    ...
    async def test_routing_service_detection(self, client):
        ...
        routing_patterns = {
            'gmail': ['list_gmail_labels', 'send_gmail_message', 'search_gmail_messages'],
    ...
    async def test_routing_chain(self, client):
        ...
        cal_result = await client.call_tool("list_calendars", {
            "user_google_email": TEST_EMAIL
        })
        ...
        print(f"✅ Middleware chain works correctly")
    ...
    async def test_routing_fallback_behavior(self, client):
        ...
        if 'health_check' in tool_names:
            ...
            print(f"   System tool 'health_check' routed successfully")
    ...
    async def test_routing_performance(self, client):
        ...
        assert discovery_time < 3.0, f"Tool discovery with routing took {discovery_time:.2f}s"
        assert call_time < 5.0, f"Routed tool call took {call_time:.2f}s"
        
        print(f"✅ Routing performance acceptable")
            ...
            try:
                ...
                if result:
                    ...
                    if case['error_type'] == 'validation':
                        assert any(word in content.lower() for word in ['error', 'required', 'missing'])
            except Exception as e:
                # Should get appropriate error
                error_msg = str(e).lower()
                if case['error_type'] == 'not_found':
                    assert any(word in error_msg for word in ['unknown', 'not found', 'invalid'])
                elif case['error_type'] == 'validation':
                    assert any(word in error_msg for word in ['validation', 'required', 'missing'])
        
        print(f"✅ Routing error handling works correctly")
...
class TestRoutingServicePriority:
    ...
    async def test_calendar_high_confidence(self, client):
        ...
        for tool_name in calendar_tools:
            if tool_name in tool_names:
                calendar_found += 1
                # These should be routed with high confidence (0.95)
                print(f"✅ Calendar tool '{tool_name}' available (95% confidence expected)")
        
        assert calendar_found > 0, "Should have Calendar tools with high confidence routing"
    ...
    async def test_routing_priority_order(self, client):
        ...
        high_confidence_result = await client.call_tool("list_gmail_labels", {
            "user_google_email": TEST_EMAIL
        })
        
        medium_confidence_result = await client.call_tool("list_spaces", {
            "user_google_email": TEST_EMAIL
        })
        ...
        assert medium_confidence_result is not None, "Medium confidence routing should work"
        
        print(f"✅ Routing priority order works correctly")
        print(f"   High confidence (Gmail): Success")
        print(f"   Medium confidence (Chat): Success")
...
class TestRoutingIntegration:
    ...
    async def test_routing_with_auth_middleware(self, client):
        ...
        print(f"✅ Routing integrates with auth middleware")
    ...
    async def test_routing_with_template_middleware(self, client):
        ...
        try:
            ...
            if result:
                ...
                print(f"✅ Routing + template resolution works")
        except Exception as e:
            # May require explicit parameter
            if 'required' in str(e).lower():
                print(f"✅ Routing validates parameters correctly")
    ...
    async def test_concurrent_routing(self, client):
        ...
        print(f"✅ Concurrent routing works")
    ...
    async def test_routing_metrics(self, client):
        ...
        print(f"✅ Routing metrics collected")
