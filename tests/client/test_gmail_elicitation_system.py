...
class TestGmailElicitationSystem:
    ...
    async def test_view_gmail_allow_list(self, client):
        ...
        result = await client.call_tool(
            ...
            {"action": "view", "user_google_email": TEST_EMAIL},
        )
    ...
    async def test_add_to_gmail_allow_list_valid_email(self, client):
        ...
        result = await client.call_tool(
            "manage_gmail_allow_list",
            {"action": "add", "user_google_email": TEST_EMAIL, "email": test_email},
        )
        ...
        assert (
            "successfully added" in content.lower()
            or "added" in content.lower()
            or "already in the allow list" in content.lower()
            or "authentication" in content.lower()
            or "credentials" in content.lower()
        )
    ...
    async def test_remove_from_gmail_allow_list_nonexistent(self, client):
        ...
        assert (
            "not in the allow list" in content.lower()
    ...
    async def test_send_gmail_message_requires_elicitation(self, client):
        ...
        assert (
            "email ready to send" in content.lower()  # Elicitation message
            or "email sent" in content.lower()  # Successful send
            or "authentication" in content.lower()  # Auth error
            or "credentials" in content.lower()
        )  # Auth error
    ...
    async def test_send_gmail_message_multiple_recipients(self, client):
        ...
        assert (
            "email ready to send" in content.lower()
            or "email sent" in content.lower()
            or "authentication" in content.lower()
            or "credentials" in content.lower()
        )
            ...
            try:
                result = await client.call_tool(
                    "send_gmail_message",
                    {
                        ...
                        "html_body": (
                            f"<p>Testing {content_type} content</p>"
    ...
    async def test_allow_list_management_workflow(self, client):
        ...
        view_result = await client.call_tool(
            "manage_gmail_allow_list",
            {"action": "view", "user_google_email": TEST_EMAIL},
        )
        ...
        remove_result = await client.call_tool(
            "manage_gmail_allow_list",
            {"action": "remove", "user_google_email": TEST_EMAIL, "email": test_email},
        )
        ...
        for result in [add_result, view_result, remove_result]:
            ...
            try:
                ...
                assert (
                    ...
                    or "allow list" in content.lower()
                    or "added" in content.lower()
                    or "removed" in content.lower()
                    or "authentication" in content.lower()
                    or "credentials" in content.lower()
    ...
    async def test_gmail_elicitation_integration(self, client):
        ...
        await client.call_tool(
            "manage_gmail_allow_list",
            {"action": "add", "user_google_email": TEST_EMAIL, "email": trusted_email},
        )

        # Step 2: Try to send to trusted email (should not trigger elicitation)
        trusted_result = await client.call_tool(
            "send_gmail_message",
            {
                "user_google_email": TEST_EMAIL,
                "to": [trusted_email],
        ...
        untrusted_result = await client.call_tool(
            "send_gmail_message",
            {
                "user_google_email": TEST_EMAIL,
                "to": [untrusted_email],
    ...
    async def test_send_gmail_message_unicode_content(self, client):
        ...
        unicode_body = (
            "Hello! 👋 This email contains unicode characters: 中文, русский, español"
        )
    ...
    async def test_resource_descriptions(self, client):
        ...
        gmail_resources = [
            resource for resource in resources if "gmail" in str(resource.uri)
        ]
    ...
    async def test_concurrent_allow_list_operations(self, client):
        ...
        for email in test_emails:
            task = client.call_tool(
                "manage_gmail_allow_list",
                {"action": "add", "user_google_email": TEST_EMAIL, "email": email},
            )
            tasks.append(task)
        ...
        for result in results:
            if isinstance(result, Exception):
                ...
                assert (
                    "authentication" in str(result).lower()
                    or "credentials" in str(result).lower()
                    or "unknown tool" in str(result).lower()
                )
            else:
