...
class TestRandomTemplateMiddleware:
    ...
    async def test_prompts_available(self, client):
        ...
        expected_prompt_categories = ["gchat", "gmail", "gsheets", "chat_app", "structured"]
        available_categories = [name for name in prompt_names if any(cat in name for cat in expected_prompt_categories)]
        
        assert len(available_categories) > 0, f"Should have prompts from expected categories. Found: {prompt_names}"
    ...
    async def test_random_template_request(self, client):
        ...
        try:
            ...
            assert result is not None, "Should get a result from random template request"
            ...
            if result.messages:
                ...
                content = first_message.content.text if hasattr(first_message.content, 'text') else str(first_message.content)
                ...
                assert has_indicators, f"Content should contain template indicators. Content: {content[:200]}"
    ...
    async def test_specific_template_categories(self, client):
                ...
                if result and result.messages:
                    ...
                    for service, indicators in service_indicators.items():
                        if service in prompt_name:
                            has_service_content = any(indicator in content for indicator in indicators)
        ...
        print(f"📊 Successfully tested {successful_tests}/{len(categories_to_test)} prompt categories")
    ...
    async def test_template_with_jinja2_features(self, client):
        ...
        try:
            ...
            if result and result.messages:
                content = result.messages[0].content.text if hasattr(result.messages[0].content, 'text') else str(result.messages[0].content)
                ...
                indicators_found = [indicator for indicator in jinja2_indicators if indicator in content]
                print(f"🎭 Jinja2 indicators found: {indicators_found}")
                
                # Should have at least some processed template content
                assert len(indicators_found) >= 2, f"Should contain Jinja2 processed content. Found: {indicators_found}"
...
if __name__ == "__main__":
    print("🧪 Run with: uv run pytest tests/client/test_random_template_middleware.py -v")