...
import pytest
import json
from .test_helpers import TestResponseValidator
...
class TestTagBasedResourceMiddlewareIntegration:
    ...
    async def test_service_resource_templates_available(self, client):
        ...
        for expected in expected_templates:
            assert expected in template_uris, f"Service resource template {expected} not found"
            print(f"   ✅ Found template: {expected}")
        
        print(f"   📊 All {len(expected_templates)} service resource templates available")
    ...
    async def test_service_lists_endpoints(self, client):
        ...
        for service in supported_services:
            try:
                ...
                assert "service_metadata" in data, f"Missing 'service_metadata' for {service}"
                ...
                print(f"   ✅ {service}: {len(data['list_types'])} list types - {list(data['list_types'].keys())}")
        ...
        print(f"   📊 {len(successful_services)}/{len(supported_services)} services working")
    ...
    async def test_service_list_items_endpoints(self, client):
        ...
        for service, list_type in test_cases:
            try:
                ...
                assert len(content) > 0, f"No content returned for {service}/{list_type}"
                ...
                if "error" in data:
                    ...
                    if any(phrase in error_msg for phrase in ["email not found", "authentication", "context"]):
                        results[f"{service}/{list_type}"] = {
                            "success": True, 
                            "auth_error": True,
                            "message": "Expected authentication error"
                        }
                        print(f"   ✅ {service}/{list_type}: Authentication error (expected)")
                    else:
                        results[f"{service}/{list_type}"] = {
                            "success": False,
                            "error": data["error"]
                        }
                        print(f"   ❌ {service}/{list_type}: Unexpected error: {data['error']}")
                else:
                    # Validate successful response
                    assert "service" in data, f"Missing 'service' field"
                    assert "list_type" in data, f"Missing 'list_type' field"
                    assert "tool_called" in data, f"Missing 'tool_called' field"
                    
                    results[f"{service}/{list_type}"] = {
                        "success": True,
                        "tool_called": data["tool_called"]
                    }
                    print(f"   ✅ {service}/{list_type}: Tool {data['tool_called']} executed")
        ...
        print(f"   📊 {len(valid_results)}/{len(test_cases)} endpoints handled correctly")
    ...
    async def test_service_specific_items_endpoints(self, client):
        ...
        for service, list_type, item_id in test_cases:
            try:
                content = await client.read_resource(f"service://{service}/{list_type}/{item_id}")
                assert len(content) > 0, f"No content returned for {service}/{list_type}/{item_id}"
                ...
                if "error" in data:
                    ...
                    if any(phrase in error_msg for phrase in ["email not found", "authentication", "context"]):
                        ...
                        print(f"   ✅ {service}/{list_type}/{item_id}: Authentication error (expected)")
                    elif "no get tool configured" in error_msg:
                        results[f"{service}/{list_type}/{item_id}"] = {
                            "success": True,
                            "no_get_tool": True
                        }
                        print(f"   ✅ {service}/{list_type}/{item_id}: No get tool (expected)")
                    else:
                        results[f"{service}/{list_type}/{item_id}"] = {
                            "success": False,
                            "error": data["error"]
                        }
                        print(f"   ❌ {service}/{list_type}/{item_id}: Unexpected error: {data['error']}")
                else:
                    # Validate successful response
                    assert "service" in data, f"Missing 'service' field"
                    assert "list_type" in data, f"Missing 'list_type' field"
                    assert "item_id" in data, f"Missing 'item_id' field"
                    assert data["item_id"] == item_id, f"Wrong item_id returned"
                    
                    results[f"{service}/{list_type}/{item_id}"] = {
                        "success": True,
                        "tool_called": data.get("tool_called")
                    }
                    print(f"   ✅ {service}/{list_type}/{item_id}: Retrieved successfully")
                
            except Exception as e:
                results[f"{service}/{list_type}/{item_id}"] = {"success": False, "error": str(e)}
        ...
        print(f"   📊 {len(valid_results)}/{len(test_cases)} specific item endpoints handled correctly")
        ...
        for uri, expected_error_type in error_test_cases:
            try:
                ...
                if len(content) > 0:
                    ...
                    print(f"   ✅ {uri}: Got expected error type '{expected_error_type}'")
                else:
    ...
    async def test_middleware_response_consistency(self, client):
        ...
        for service in test_services:
            try:
                ...
                required_fields = ["service", "service_metadata", "list_types", "total_list_types", "generated_at"]
                
                for field in required_fields:
                    assert field in data, f"Missing required field '{field}' for {service}"
                ...
                for field in metadata_fields:
                    assert field in metadata, f"Missing metadata field '{field}' for {service}"
                
                # Validate list types structure
                for list_type_name, list_type_info in data["list_types"].items():
                    list_type_fields = ["display_name", "description", "tool_name", "supports_get", "id_field"]
                    
                    for field in list_type_fields:
                        assert field in list_type_info, f"Missing list type field '{field}' for {service}/{list_type_name}"
        ...
        print(f"   📊 All tested services have consistent response formats")
...
class TestTagBasedResourceMiddlewarePerformance:
    ...
    async def test_concurrent_service_requests(self, client):
        ...
        async def fetch_service_lists(service):
            try:
                content = await client.read_resource(f"service://{service}/lists")
                return {"service": service, "success": True, "content_length": len(content)}
            except Exception as e:
        ...
        print(f"   ✅ {len(successful_requests)}/{len(services)} concurrent requests succeeded")
    ...
    async def test_repeated_requests_consistency(self, client):
        ...
        if "error" not in first_result:
            for i, result in enumerate(results[1:], 2):
                if "error" not in result:
                    ...
                    assert result["list_types_count"] == first_result["list_types_count"], \
                        f"List types count mismatch on request {i}"
                    assert set(result["list_types"]) == set(first_result["list_types"]), \
        ...
        if successful_requests:
            print(f"   📊 Consistent response: {successful_requests[0]['list_types_count']} list types")
