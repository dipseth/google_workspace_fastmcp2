...
class TestSlidesTools:
            ...
            try:
                async with client:
                    ...
                    if result and len(result) > 0:
                        ...
                        print(
                            f"Create presentation response: {content}"
                        )  # Debug output
                        ...
                        if "presentation created successfully" in content.lower():
                            ...
                            for pattern in patterns:
                                ...
                                if match:
                                    ...
                                    if (
                                        len(presentation_id) >= 20
                                        and len(presentation_id) <= 50
                                    ):
                                        TestSlidesTools._shared_presentation_id = (
                                            presentation_id
                                        )
                                        print(
                                            f"Extracted presentation ID: {presentation_id}"
                                        )
    ...
    async def test_create_presentation(self, client):
        ...
        result = await client.call_tool(
            "create_presentation",
            {"user_google_email": TEST_EMAIL, "title": "Test Presentation from MCP"},
        )
        ...
        assert (
            "requires authentication" in content.lower()
            or "no valid credentials" in content.lower()
            or "presentation created successfully" in content.lower()
            or "presentationid" in content.lower()
        )
    ...
    async def test_create_presentation_minimal(self, client):
        ...
        result = await client.call_tool(
            "create_presentation",
            {"user_google_email": TEST_EMAIL, "title": "Minimal Test Presentation"},
        )
    ...
    async def test_get_presentation_info(self, client, test_presentation_id):
        ...
        if not test_presentation_id:
            pytest.skip(
                "No test presentation ID available - authentication required or presentation creation failed"
            )

        result = await client.call_tool(
            "get_presentation_info",
            {"user_google_email": TEST_EMAIL, "presentation_id": test_presentation_id},
        )
    ...
    async def test_get_presentation_file(self, client, test_presentation_id):
        ...
        if any(
            keyword in content.lower()
            for keyword in ["requires authentication", "no valid credentials"]
        ):
            ...
            try:
                ...
                print(
                    f"Warning: Could not clean up test file {local_file_path}: {cleanup_error}"
                )
        else:
    ...
    @pytest.mark.asyncio
    async def test_get_presentation_file_different_formats(
        self, client, test_presentation_id
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_presentation_file_invalid_format(
        self, client, test_presentation_id
    ):
...
class TestSlidesIntegration:
    ...
    async def test_slides_error_handling(self, client):
        ...
        result = await client.call_tool(
            "get_presentation_info",
            {"user_google_email": TEST_EMAIL, "presentation_id": "invalid_id_12345"},
        )
