...
class TestQdrantRefactoredMiddleware:
    ...
    async def test_middleware_imports_correctly(self, client):
        ...
        from middleware.qdrant_middleware import QdrantUnifiedMiddleware, setup_enhanced_qdrant_tools
        
        # Test backward compatibility aliases
        from middleware.qdrant_middleware import QdrantResponseMiddleware, EnhancedQdrantResponseMiddleware
...
class TestQdrantRefactoredTools:
    ...
    async def test_legacy_tools_still_work(self, client):
        ...
        result = await client.call_tool("search_tool_history", {
            "query": "test legacy compatibility",
            "limit": 5
        })
...
class TestQdrantResourceHandling:
        ...
        try:
            ...
            if isinstance(content, list) and len(content) > 0:
                ...
                if "error" in json_data:
                    ...
                    assert hasattr(pydantic_obj, 'qdrant_enabled'), "Should have qdrant_enabled field"
                    assert hasattr(pydantic_obj, 'collections'), "Should have collections field"
                    assert hasattr(pydantic_obj, 'total_collections'), "Should have total_collections field"
                ...
                raise AssertionError(f"Expected list with resource content, got: {type(content)}")

        except Exception as e:
            print(f"❌ Exception occurred: {type(e).__name__}: {e}")
            # Should not be tuple/attribute errors anymore
            error_msg = str(e).lower()
            print(f"🔍 Error message check - contains 'tuple': {'tuple' in error_msg}, contains 'attribute': {'attribute' in error_msg}")
    ...
    async def test_qdrant_search_resource_returns_pydantic(self, client):
        ...
        try:
            ...
            if isinstance(content, QdrantSearchResponse):
                ...
                assert hasattr(content, 'total_results'), "Should have total_results field"
    ...
    async def test_context_caching_mechanism(self, client):
        ...
        middleware.resource_handler.handle_qdrant_resource = AsyncMock(return_value=mock_result)
...
class TestQdrantRefactoredPerformance:
    ...
    async def test_tool_response_time(self, client):
        ...
        for query in test_queries:
            ...
            assert response_time < 10.0, f"Search for '{query}' took {response_time:.2f}s, should be < 10s"
    ...
    class TestQdrantDataSanitization:
        ...
        async def test_middleware_handles_binary_data(self, client):
            ...
            try:
                ...
                assert "utf-8" not in str(e).lower(), f"UTF-8 error still occurring: {e}"
    ...
    class TestQdrantResourceRegistration:
        ...
        async def test_setup_qdrant_resources_function(self, client):
            ...
            assert callable(setup_qdrant_resources), "setup_qdrant_resources should be callable"
            ...
            assert mock_mcp.resource.call_count >= 6, "Should register at least 6 qdrant:// resources"
        ...
        async def test_qdrant_status_resource(self, client):
            ...
            try:
                ...
                error_msg = str(e).lower()
                assert any(word in error_msg for word in ["not available", "connection", "tuple", "attribute"]), \
    ...
    class TestQdrantActualDataOperations:
        ...
        async def test_actual_middleware_storage_integration(self, client):
            ...
            try:
                ...
                assert "utf-8" not in str(e).lower(), f"UTF-8 error in actual storage: {e}"
        ...
        async def test_middleware_handles_invalid_responses(self, client):
            ...
            for i, response in enumerate(test_responses):
                try:
                    ...
                    assert "utf-8" not in str(e).lower(), f"UTF-8 error with response {i}: {e}"
    ...
    class TestQdrantToolIntegration:
        ...
        async def test_fetch_tool_with_actual_middleware(self, client):
            ...
            for test_id in test_ids:
                try:
                    result = await client.call_tool("fetch", {"point_id": test_id})
                    assert result is not None, f"FETCH should return result for ID: {test_id}"
                    ...
                    try:
                        ...
                        assert "error" in content.lower(), f"Invalid response for ID {test_id}: {content}"
                    
                except Exception as e:
                    # Should not be UTF-8 errors
                    assert "utf-8" not in str(e).lower(), f"UTF-8 error with FETCH for ID {test_id}: {e}"
        ...
        async def test_search_tool_with_actual_middleware(self, client):
            ...
            for query in test_queries:
                try:
                    result = await client.call_tool("search", {"query": query})
                    assert result is not None, f"SEARCH should return result for query: {query}"
                    ...
                    try:
                        data = json.loads(content)
                        assert "results" in data, f"SEARCH response must have 'results' field for query: {query}"
                        assert isinstance(data["results"], list), "Results should be a list"
                    ...
                    # Should not be UTF-8 errors
                    assert "utf-8" not in str(e).lower(), f"UTF-8 error with SEARCH for query '{query}': {e}"
        ...
        async def test_concurrent_storage_operations(self, client):
            ...
            async def store_test_data(i):
                try:
                    ...
                    # Should not be UTF-8 errors
                    assert "utf-8" not in str(e).lower(), f"UTF-8 error in concurrent test {i}: {e}"
                    return False
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    # Should not be UTF-8 errors
                    assert "utf-8" not in str(result).lower(), f"UTF-8 error in result {i}: {result}"
        ...
        async def test_memory_usage_stability(self, client):
            ...
            for i in range(10):
                try:
                    ...
                    # Should not be UTF-8 errors
                    assert "utf-8" not in str(e).lower(), f"UTF-8 error in memory test {i}: {e}"
