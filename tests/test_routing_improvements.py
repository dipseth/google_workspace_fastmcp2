...
class MockFastMCP:
    ...
    def add_mock_tool(self, name: str, tags: List[str], required_scopes: List[str] = None):
...
class TestServiceRouter:
    ...
    def test_direct_tool_mapping(self):
        ...
        assert decision.primary.confidence_score >= 0.9  # High confidence for direct mapping
    ...
    def test_pattern_matching(self):
        ...
        assert decision.primary.service == "gmail"
        assert decision.primary.confidence_score >= 0.7  # Good confidence for pattern match
    ...
    def test_parameter_analysis(self):
        ...
        assert decision.primary.confidence_score >= 0.3  # Lower confidence for parameter analysis
    ...
    def test_confidence_scoring(self):
        ...
        high_conf_decision = self.service_router.identify_service_from_request(high_conf_request)
        
        # Low confidence case
        low_conf_request = {"method": "unknown_method", "params": {}}
        low_conf_decision = self.service_router.identify_service_from_request(low_conf_request)
        
        assert high_conf_decision.primary.confidence_score > low_conf_decision.primary.confidence_score
    ...
    def test_routing_validation(self):
        ...
        validation = self.service_router.validate_routing_decision(decision, available_scopes)
        ...
        validation_bad = self.service_router.validate_routing_decision(decision, insufficient_scopes)
    ...
    def test_dynamic_service_registration(self):
        ...
        gmail_service = next(svc for svc in available_services if svc['name'] == 'gmail')
...
class TestEnhancedServiceDispatcher:
    ...
    async def test_fallback_routing(self, mock_get_service):
        ...
        def mock_service_side_effect(*args, **kwargs):
            service_type = kwargs.get('service_type', args[1] if len(args) > 1 else None)
        ...
        try:
            ...
            assert result.metadata.get('fallback_used', False) or result.confidence_score < 1.0
    ...
    def test_service_suggestions(self):
        ...
        gmail_suggestion = next((s for s in suggestions if s['service']['name'] == 'gmail'), None)
...
class TestServiceCategorizer:
    ...
    def test_service_discovery(self):
        ...
        assert 'write' in gmail_info['operations']  # 'send' maps to 'write' operation type
    ...
    def test_service_capabilities(self):
        ...
        assert 'write' in capabilities['operations']  # 'send' maps to 'write' operation type
...
class TestIntegrationScenarios:
    ...
    def setup_method(self):
        ...
        tools_data = [
            ("send_gmail_message", ["gmail", "send", "email"], ["https://www.googleapis.com/auth/gmail.send"]),
            ("list_gmail_messages", ["gmail", "read", "email"], ["https://www.googleapis.com/auth/gmail.readonly"]),
            ("upload_to_drive", ["drive", "upload", "file"], ["https://www.googleapis.com/auth/drive.file"]),
            ("search_drive_files", ["drive", "search", "file"], ["https://www.googleapis.com/auth/drive.readonly"]),
            ("create_event", ["calendar", "create", "event"], ["https://www.googleapis.com/auth/calendar.events"]),
            ("list_events", ["calendar", "list", "event"], ["https://www.googleapis.com/auth/calendar.readonly"])
        ]
    ...
    def test_end_to_end_routing(self):
        ...
        test_requests = [
            {"method": "send_gmail_message", "expected_service": "gmail", "expected_confidence": 0.9},
            {"method": "upload_to_drive", "expected_service": "drive", "expected_confidence": 0.9},
            {"method": "create_event", "expected_service": "calendar", "expected_confidence": 0.9},
            {"method": "gmail_search", "expected_service": "gmail", "expected_confidence": 0.7},
            {"method": "unknown_method", "expected_service": None, "expected_confidence": 0.3}
        ]
        ...
        for test_req in test_requests:
            ...
            if test_req["expected_service"]:
                ...
                assert decision.primary.confidence_score >= test_req["expected_confidence"]
            else:
    ...
    def test_metadata_consistency(self):
        ...
        routing_services = {svc['name'] for svc in self.service_router.get_available_services()}
        categorizer_services = set(self.dispatcher.categorizer.discover_services_from_tools().keys())
