...
from gchat.enhanced_card_tool import (
    ...
    _ensure_async_components_initialized,
    send_enhanced_card,
    save_card_template,
    find_card_templates,
    validate_card
...
class TestEnhancedCardTool:
    ...
    def mock_template_manager(self):
        ...
        mock_manager.get_template.return_value = {
            ...
            "placeholders": {
                "content": "sections.0.widgets.0.textParagraph.text"
            }
        }
    ...
    @pytest.mark.asyncio
    async def test_component_initialization(self, mock_content_mapping_engine, mock_parameter_inference_engine,
                                           mock_template_manager, mock_widget_specification_parser, mock_card_validator):
        """Test that components are properly initialized."""
        with patch('gchat.enhanced_card_tool._content_mapping_engine', None), \
             ...
             patch('gchat.enhanced_card_tool.ContentMappingEngine', return_value=mock_content_mapping_engine), \
             patch('gchat.enhanced_card_tool.ParameterInferenceEngine', return_value=mock_parameter_inference_engine), \
             patch('gchat.enhanced_card_tool.TemplateManager', return_value=mock_template_manager), \
             patch('gchat.enhanced_card_tool.WidgetSpecificationParser', return_value=mock_widget_specification_parser), \
             patch('gchat.enhanced_card_tool.CardValidator', return_value=mock_card_validator):
            
            # Call the initialization function
            _ensure_components_initialized()
            
            # Check that components were initialized
            from gchat.enhanced_card_tool import (_content_mapping_engine, _parameter_inference_engine,
    ...
    @pytest.mark.asyncio
    async def test_send_enhanced_card(self, mock_content_mapping_engine, mock_parameter_inference_engine,
                                     mock_template_manager, mock_widget_specification_parser, mock_card_validator):
        """Test sending an enhanced card."""
        # Mock the components and functions
        with patch('gchat.enhanced_card_tool._content_mapping_engine', mock_content_mapping_engine), \
             patch('gchat.enhanced_card_tool._parameter_inference_engine', mock_parameter_inference_engine), \
             patch('gchat.enhanced_card_tool._template_manager', mock_template_manager), \
             patch('gchat.enhanced_card_tool._widget_specification_parser', mock_widget_specification_parser), \
             patch('gchat.enhanced_card_tool._card_validator', mock_card_validator), \
             patch('gchat.enhanced_card_tool._find_card_component') as mock_find_component, \
             patch('gchat.enhanced_card_tool._create_card_from_component') as mock_create_card, \
             patch('gchat.enhanced_card_tool._convert_card_to_google_format') as mock_convert_card, \
             patch('gchat.enhanced_card_tool._get_chat_service_with_fallback') as mock_get_service, \
            ...
            mock_convert_card.return_value = {"card": {"header": {"title": "Test Card"}}}
            
            mock_message = MagicMock()
            mock_message.render.return_value = {"text": "Test message", "cardsV2": [{"card": {"header": {"title": "Test Card"}}}]}
            ...
            mock_execute.return_value = {"name": "messages/123", "createTime": "2025-07-03T10:00:00Z"}
            ...
            mock_card_validator.validate_card_structure.return_value = (False, ["Missing required field"])
    ...
    async def test_save_card_template(self, mock_template_manager):
        ...
        with patch('gchat.enhanced_card_tool._template_manager', mock_template_manager), \
    ...
    async def test_find_card_templates(self, mock_template_manager):
        ...
        with patch('gchat.enhanced_card_tool._template_manager', mock_template_manager), \
            ...
            # Check that the template manager was called correctly
            mock_template_manager.find_templates.assert_called_once_with("test template", 5)
    ...
    async def test_validate_card(self, mock_card_validator):
        ...
        with patch('gchat.enhanced_card_tool._card_validator', mock_card_validator), \
            ...
            mock_card_validator.validate_card_structure.assert_called_once_with(test_card)
            mock_card_validator.auto_fix_common_issues.assert_not_called()  # Should not be called if validation passes
            ...
            # Check that the validator was called correctly
            mock_card_validator.validate_card_structure.assert_called_once_with(test_card)
            mock_card_validator.auto_fix_common_issues.assert_called_once_with(test_card)
            mock_card_validator.suggest_improvements.assert_called_once()
