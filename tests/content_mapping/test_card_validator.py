...
class TestCardValidator(unittest.TestCase):
    ...
    def test_validate_card_structure_invalid_missing_required(self):
        ...
        self.assertTrue(any("Missing required header field: title" in issue for issue in issues))
        self.assertTrue(any("Missing required field in section 0: widgets" in issue for issue in issues))
    ...
    def test_validate_card_structure_invalid_empty(self):
        ...
        self.assertFalse(is_valid)
        self.assertGreater(len(issues), 0)
        self.assertTrue(any("Card must have either header or sections" in issue for issue in issues))
    ...
    def test_validate_widget_invalid_missing_required(self):
        ...
        self.assertFalse(is_valid)
        self.assertGreater(len(issues), 0)
        self.assertTrue(any("Missing required field 'text'" in issue for issue in issues))
    ...
    def test_auto_fix_common_issues_missing_alt_text(self):
        ...
        self.assertEqual(
            fixed_card["sections"][0]["widgets"][0]["image"]["altText"],
            "Image"
        )
    ...
    def test_auto_fix_common_issues_empty_button_list(self):
        ...
        self.assertEqual(len(fixed_card["sections"][0]["widgets"][0]["buttonList"]["buttons"]), 1)
        self.assertEqual(
    ...
    def test_suggest_improvements_minimal_card(self):
        ...
        self.assertTrue(any("header" in suggestion.lower() for suggestion in suggestions))
        self.assertTrue(any("interactive" in suggestion.lower() for suggestion in suggestions))
        self.assertTrue(any("image" in suggestion.lower() for suggestion in suggestions))
