...
def test_oauth_proxy_registration():
    ...
    try:
        ...
        if not settings.is_oauth_configured():
            ...
            print(
                "   Set GOOGLE_CLIENT_SECRETS_FILE or GOOGLE_CLIENT_ID/SECRET to test"
            )
            return False
        ...
        real_creds = oauth_proxy.get_real_credentials(
            temp_client_id, temp_client_secret
        )

        if real_creds:
            mapped_real_id, mapped_real_secret = real_creds
            print("   ✅ Successfully mapped temp credentials to real ones")
        ...
        invalid_result = oauth_proxy.get_real_credentials(
            temp_client_id, "wrong_secret"
        )
        assert invalid_result is None, "❌ ERROR: Should not map with wrong secret"
        print("   ✅ Correctly rejects invalid temporary secret")

        # Test with non-existent client
        invalid_result = oauth_proxy.get_real_credentials(
            "mcp_nonexistent", "any_secret"
        )
        assert invalid_result is None, "❌ ERROR: Should not map non-existent client"
        print("   ✅ Correctly rejects non-existent client")
...
def test_token_exchange_simulation():
    ...
    try:
        ...
        if real_creds:
            ...
            print("   ✅ Real credentials would be used with Google OAuth")
            print("   ✅ Token exchange would succeed")
        else:
            print("   ❌ Failed to map credentials")
            return False
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            async with session.get(
                ...
                print("   ✅ Protected resource endpoint accessible")
            ...
            async with session.get(
                ...
                print("   ✅ Authorization server endpoint accessible")
                print(
                    f"   ✅ Token endpoint points to local proxy: {data.get('token_endpoint')}"
                )
            ...
            async with session.post(
                ...
                print(f"   ✅ Temp Client ID: {temp_client_id}")
                print("   ✅ Proxy credentials generated (real creds hidden)")
            ...
            print("   ✅ OAuth Proxy is active (client registered successfully)")
            print(f"   ✅ Proxy issued temp credentials: {temp_client_id}")
            print("   ✅ Registration proves proxy is managing clients")
            ...
            async with session.post(
                ...
                print("   ✅ Token endpoint accessible and validates requests")
            ...
            print(
                "   ℹ️  Test client will be cleaned up on server restart or after expiry"
            )
...
def test_oauth_proxy_lifecycle():
    ...
    try:
        ...
        # Get initial stats
        stats = oauth_proxy.get_stats()
        print("   ✅ OAuth Proxy stats accessible")
        ...
        print(
            "   ✅ OAuth endpoints registered via setup_oauth_endpoints_fastmcp (line 641)"
        )
        print(
            "   ✅ OAuth discovery available BEFORE authentication setup (lines 639-649)"
        )
        print(
            "   ✅ Authentication configured AFTER discovery endpoints (lines 655-660)"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"2. Token Exchange Simulation: {'✅ PASSED' if test2_passed else '❌ FAILED'}"
    )
    print(
        f"3. OAuth Proxy Lifecycle:     {'✅ PASSED' if test3_passed else '❌ FAILED'}"
    )
    print(
        f"4. Server Integration:        {'✅ PASSED' if test4_passed else '⚠️ SKIPPED/FAILED'}"
    )
