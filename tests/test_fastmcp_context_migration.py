...
def test_service_injection_functions(mock_context):
    ...
    with patch('auth.context.get_context', return_value=mock_context):
        ...
        assert mock_context._state["service_requests"]["drive"]["service"] == mock_service
...
def test_context_outside_request():
    ...
    with patch('auth.context.get_context', side_effect=RuntimeError("not in a FastMCP request context")):
