...
The Enhanced Tools leverage Groupon Google MCP's resource templating feature to automatically inject authentication credentials, eliminating the need for `user_google_email` parameters in every request. These tools provide a streamlined, more intuitive API experience.
...
Enhanced tools use Groupon Google MCP's resource templating system to automatically inject user context. Here's how it works:
