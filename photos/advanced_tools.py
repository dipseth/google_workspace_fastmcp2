...
from .photos_types import (
    PhotoUploadResponse, PhotosSmartSearchResponse, PhotosBatchDetailsResponse,
    PhotosPerformanceStatsResponse, PhotosAlbumSyncResponse, PhotosFolderUploadResponse,
    PhotoInfo
)
...
def setup_advanced_photos_tools(mcp: FastMCP) -> None:
    ...
    async def photos_performance_stats(
        ...
        try:
            ...
            return PhotosPerformanceStatsResponse(
                ...
                cache_utilization_percent=(stats['cache_size'] / stats['max_size'] * 100),
    ...
    async def upload_photos(
        ...
        logger.info(f"[upload_photos] User: {user_google_email}, Files: {len(file_list)}, Single: {is_single_upload}")
        ...
        try:
            ...
            if is_single_upload:
                ...
                if results['successful']:
                    ...
                    if len(results['successful']) > 10:
                        text_output += f"... and {len(results['successful']) - 10} more\n"
                ...
                if results['failed']:
                    ...
                    if len(results['failed']) > 5:
                        text_output += f"... and {len(results['failed']) - 5} more failures\n"
            ...
            logger.info(f"Photo upload completed: {successful_count}/{total_count} successful")
    ...
    async def upload_folder_photos(
        ...
        """
        start_time = datetime.now()
        logger.info(f"[upload_folder_photos] User: {user_google_email}, Folder: {folder_path}")
        ...
        try:
            ...
            if album_id and not results.get('album_id'):
                ...
                logger.info(f"Note: Adding to existing album {album_id} not yet implemented")
            ...
            if results['successful']:
                ...
                if len(results['successful']) > 8:
                    text_output += f"... and {len(results['successful']) - 8} more photos\n"
            ...
            logger.info(f"Folder upload completed: {successful_count}/{total_count} photos uploaded")
