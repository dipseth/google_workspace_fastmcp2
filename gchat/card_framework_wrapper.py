...
def _generate_gchat_symbols() -> Dict[str, str]:
    ...
    if wrapper.client:
        try:
            ...
            if symbols:
                logger.info(f"🔣 Loaded {len(symbols)} symbols from Qdrant (SSoT)")
                return symbols
    ...
    if wrapper.components:
        component_names = [
            comp.name for comp in wrapper.components.values()
...
def find_component_by_symbol(symbol: str) -> Optional[Dict]:
    ...
    wrapper = get_card_framework_wrapper()
    results = wrapper.search_by_text(component_name, field="name", limit=1, is_phrase=True)
...
def get_dsl_documentation(include_examples: bool = True, include_hierarchy: bool = True) -> str:
    """
    ...
    if include_hierarchy and relationships:
        ...
        lines.append("Shows which components can contain which (parent → children):\n")
        ...
        for parent, children in sorted(relationships.items()):
            ...
            lines.append(f"- {parent_sym} **{parent}** → [{', '.join(child_syms)}]")

    # Build syntax examples using actual symbols
    section_sym = symbols.get("Section", "§")
    button_sym = symbols.get("Button", "ᵬ")
    dtext_sym = symbols.get("DecoratedText", "đ")
    grid_sym = symbols.get("Grid", "ℊ")
    gitem_sym = symbols.get("GridItem", "ǵ")

    lines.append("\n### DSL Syntax\n")
    lines.append(f"- `{section_sym}[{dtext_sym}, {button_sym}]` → Section with DecoratedText and Button")
    lines.append(f"- `{button_sym}×2` → 2 Buttons (multiplier notation)")
    lines.append(f"- `{grid_sym}[{gitem_sym}×4]` → Grid with 4 GridItems")
    ...
    if include_examples:
        ...
        lines.append(f'structure_dsl="{section_sym}[{grid_sym}[{gitem_sym}×6]]"')
        lines.append("```")
...
def get_dsl_field_description() -> str:
    ...
    key_mappings = []
    key_components = ["Section", "DecoratedText", "Button", "ButtonList",
                      "Grid", "GridItem", "Image", "Icon", "TextInput", "OnClick"]
    ...
    return (
        ...
        f"Example: '§[đ, Ƀ[ᵬ×2]]' = Section with DecoratedText and 2 Buttons. "
...
def get_component_relationships_for_dsl() -> Dict[str, List[str]]:
    ...
    if wrapper.client:
        try:
            ...
            logger.info(f"📋 Loaded {len(relationships)} component relationships for DSL")
    ...
    return {
        ...
        "Section": ["DecoratedText", "TextParagraph", "ButtonList", "Grid",
                    "Divider", "Image", "Columns", "SelectionInput",
                    "TextInput", "DateTimePicker", "ChipList"],
        "Columns": ["Column"],
...
def dsl_to_qdrant_queries(dsl_string: str, collection_name: Optional[str] = None) -> List[Dict]:
    """
