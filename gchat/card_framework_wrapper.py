...
CARD_WIDGET_TYPES = {
    "DecoratedText", "TextParagraph", "Image", "ButtonList", "ChipList",
    "Grid", "SelectionInput", "DateTimePicker", "Divider", "TextInput",
    "Columns", "SwitchControl", "CollapseControl",
}

# Form components that need a 'name' field
CARD_FORM_COMPONENTS = {
    "TextInput", "SelectionInput", "DateTimePicker", "SwitchControl",
}

# Empty components (just structure, no content params)
CARD_EMPTY_COMPONENTS = {
    "Divider", "CollapseControl",
}
...
def get_dsl_documentation(
    ...
    lines.append("**Card:** " + ", ".join(
        f"{symbols.get(c, '?')}={c}" for c in CORE_COMPONENTS["card_structure"] if c in symbols
    ))
    lines.append("**Carousel:** " + ", ".join(
        f"{symbols.get(c, '?')}={c}" for c in CORE_COMPONENTS["carousel"] if c in symbols
    ))
    lines.append("**Containers:** " + ", ".join(
        f"{symbols.get(c, '?')}={c}" for c in CORE_COMPONENTS["containers"] if c in symbols
    ))
    lines.append("**Widgets:** " + ", ".join(
        f"{symbols.get(c, '?')}={c}" for c in CORE_COMPONENTS["widgets"] if c in symbols
    ))
    lines.append("**Items:** " + ", ".join(
        f"{symbols.get(c, '?')}={c}" for c in CORE_COMPONENTS["items"] if c in symbols
    ))
    ...
    if include_hierarchy:
        ...
        lines.append(f"- {section_sym} Section → {dtext_sym} {tpara_sym} {image_sym} {buttonlist_sym} {chiplist_sym} {grid_sym} {columns_sym}")
        lines.append(f"- {carousel_sym} Carousel → {ccard_sym} CarouselCard → {nested_sym} NestedWidget → {tpara_sym} {buttonlist_sym} {image_sym}")
        lines.append(f"- {buttonlist_sym} ButtonList → {button_sym}, {chiplist_sym} ChipList → {chip_sym}")
        lines.append(f"- {grid_sym} Grid → {gitem_sym}, {columns_sym} Columns → {column_sym}")
    ...
    if include_examples:
        ...
        lines.append(f"- `{section_sym}[{dtext_sym}, {buttonlist_sym}[{button_sym}×2]]` → Text + 2 buttons")
        lines.append(f"- `{section_sym}[{grid_sym}[{gitem_sym}×4]]` → Grid with 4 items")
    ...
    lines.append("\n### More Info")
    lines.append("Read `skill://gchat-cards/` resources for complete docs (100+ components).")
...
def get_dsl_field_description() -> str:
    ...
    key_components = [
        ...
        "Grid", "GridItem", "Carousel", "CarouselCard", "NestedWidget",
    ]
...
def _generate_gchat_dsl_template(wrapper) -> str:
    ...
    symbol_table = wrapper.get_symbol_table_text() if hasattr(wrapper, "get_symbol_table_text") else "No symbols available."
